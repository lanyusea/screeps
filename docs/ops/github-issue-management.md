...
Any task closure evidence must also include the reusable `Universal task Done gate`: task type, expected observable outcome / named deliverable, non-goals or accepted substitutes, required verification evidence, Project `Evidence` / `Next action` / `Blocked by` state, post-merge/deploy/runtime proof, and named deliverable proof. If the issue names a concrete surface such as Grafana, GitHub Pages, a Discord route, deployed service, public URL, specific monitor, dashboard, or report, closure requires proof of that named surface unless Project `Evidence` records an owner-accepted substitute.

The normal project PR gates still apply: worktree branch, no direct `main` edits, at least 15 minutes after PR creation before merge, all discussions resolved, automated review口径 satisfied (CodeRabbit/Gemini no blocking findings or recorded reliability bypass; formal GitHub approval is not required), and green required checks once configured. A CodeRabbit/Gemini bypass is allowed only when the reviewer is unreliable/unavailable/rate-limited/skipped/stale/non-substantive for the exact PR head and the PR/Project evidence records reviewer, head SHA, reason, required-check status, and review-thread state. Every active agent PR must be added to Project `screeps` and keep `Status`, `Evidence`, and `Next action` current until it is merged or explicitly closed.
