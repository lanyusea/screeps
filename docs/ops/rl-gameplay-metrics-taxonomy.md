...
| `territory.rcl_level` | Survival/ownership | Track room controller level. | `rclLevel`, `controller.level`. | Higher is better | RCL progression is durable room capability evidence. | Historical rows stay `NULL` when absent. | Repeated absence promotes controller telemetry work. |
| `territory.controller_progress_ratio` | Survival/ownership | Track controller progress within the current RCL. | `controller.progressRatio` or `controller.progress / controller.progressTotal`. | Higher is better | Flat values with blocked upgrade energy identify controller progression stalls. | Historical rows stay `NULL` when absent. | Repeated absence during upgrade windows promotes controller-progress telemetry work. |
...
| `economy.import_demand` | Resource economy | Track multi-room energy demand that should be satisfied by imports. | `resources.multiRoomEnergy.importDemand`. | Lower is better once supply routes exist | Persistent demand identifies E29N56-style colony supply gaps. | Historical rows stay `NULL` when absent. | Repeated positive demand without imports promotes multi-room logistics work. |
| `economy.blocked_import_energy` | Resource economy | Track import demand blocked by routing, reservations, or exporter constraints. | `resources.multiRoomEnergy.blockedImportEnergy`. | Lower is better | Positive values mean needed imported energy was not satisfiable. | Historical rows stay `NULL` when absent. | Repeated positive blocked energy promotes multi-room logistics or reservation work. |
| `economy.multi_room_deficit_energy` | Resource economy | Track local energy deficit before imports/exports are balanced. | `resources.multiRoomEnergy.deficitEnergy`. | Lower is better | Positive deficits explain rooms dependent on imports or local production improvements. | Historical rows stay `NULL` when absent. | Repeated high deficits promote supply, harvest, or import routing work. |
...
| `controller.upgrade_carried_energy` | Gameplay behavior | Track energy carried by workers assigned to controller upgrade work. | `resources.productiveEnergy.upgradeCarriedEnergy`. | Higher when controller progress is intended | `0` with low controller progress identifies upgrade blockage or starvation. | Historical rows stay `NULL` when absent. | Repeated zero with low progress promotes upgrader assignment or supply work. |
