...
6. P1 has its own intra-priority fairness layer so one P1 domain cannot monopolize all normal P1 capacity. On every run, compute the oldest executable `Ready` P1 issue by Project `Domain`. Any owner-visible/repeated P1 defect older than 24 hours, or any executable non-gameplay P1 older than 36 hours, is an **aged P1**. If the gameplay floor is already satisfied and no hard P0, official-deploy blocker, or immediately mergeable PR gate must be handled first, the scheduler must dispatch one bounded aged-P1 lane before creating another new same-domain Territory/Economy gameplay P1 lane.
7. If an aged P1 is not selected, the scheduler checkpoint and the issue's Project `Evidence` / `Next action` must name the exact blocker and the next guaranteed opportunity. Generic `deferred`, `no urgency`, or silent Ready-list carryover is non-compliant.
8. P1 domain fairness is a scheduling rule, not a label rewrite. It does not change the GitHub priority label and does not permit direct Hermes edits to production/test/build code; production code remains Codex-owned.
9. For each scheduler decision, write the selected cycle slot, skipped higher-priority work, consumed/created P2 credit, P1 domain-fairness state, and exact next action into GitHub Project `Evidence` / `Next action` or the scheduler checkpoint. Redact all sensitive details in those fields and checkpoints: never copy secrets, credentials, token-bearing URLs, local secret paths, or raw command/config snippets that may contain credentials; summarize sanitized evidence instead.

Selection inside a priority bucket is ordered by: executable unblock step first, stale age, P1 domain fairness when applicable, project vision chain (`territory > resources > kills`), open PR before new issue, then the smallest independently verifiable slice.
...
2. **Close ready loops.** If a PR has completed QA, green required checks, resolved/outdated review threads, and the >=15 minute automated review gate (including no blocking automated findings or a recorded CodeRabbit/Gemini reliability bypass), merge it, fast-forward `/root/screeps`, and set the linked issue/PR Project items to `Done` with evidence.
