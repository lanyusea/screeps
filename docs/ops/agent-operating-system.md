...
- Acceptance-first issue closure: do not use a GitHub closing keyword or move an issue to `Done` unless every original acceptance criterion is satisfied and no post-merge/runtime/owner-action/successor/partial-fix blocker remains. Avoid negated close-keyword phrases such as `does not close #123`; write `Related to issue 123` for non-closing linkage.
