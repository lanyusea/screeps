...
```bash
python3 scripts/screeps-runtime-monitor.py summary --room shardX/E24S49
```
...
python3 scripts/screeps-runtime-monitor.py alert --room shardX/E24S49 --force-alert-image
```
...
```bash
python3 scripts/screeps-runtime-monitor.py alert --room shardX/E24S49 \
  | python3 scripts/screeps-runtime-monitor.py tactical-response
```
...
```bash
printf '%s\n' '{"ok":true,"mode":"alert","alert":false,"reasons":[],"rooms":["shardX/E24S49"],"warnings":[]}' \
  | python3 scripts/screeps-runtime-monitor.py tactical-response
```
...
```bash
printf '%s\n' '{"ok":true,"mode":"alert","alert":true,"reasons":[{"kind":"hostile_creep","room":"shardX/E24S49","object_id":"hostile-1","owner":"Invader","x":20,"y":21,"message":"hostile creep visible: Invader at 20,21"}],"rooms":["shardX/E24S49"]}' \
  | python3 scripts/screeps-runtime-monitor.py tactical-response
```
...
```bash
# Load SCREEPS_AUTH_TOKEN into the environment without printing it.
python3 scripts/screeps-runtime-monitor.py summary --room shardX/E24S49 --out-dir /root/screeps/runtime-artifacts/screeps-monitor-smoke
SCREEPS_MONITOR_STATE_FILE=/tmp/screeps-monitor-alert-smoke-state.json \
  python3 scripts/screeps-runtime-monitor.py alert --room shardX/E24S49 --out-dir /root/screeps/runtime-artifacts/screeps-monitor-smoke
```
