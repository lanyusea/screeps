...
```bash
python3 scripts/screeps-runtime-monitor.py summary --room shardX/E29N55
```
...
python3 scripts/screeps-runtime-monitor.py alert --room shardX/E29N55 --force-alert-image
```
...
```bash
python3 scripts/screeps-runtime-monitor.py alert --room shardX/E29N55 \
  | python3 scripts/screeps-runtime-monitor.py tactical-response
```
...
```bash
printf '%s\n' '{"ok":true,"mode":"alert","alert":false,"reasons":[],"rooms":["shardX/E29N55"],"warnings":[]}' \
  | python3 scripts/screeps-runtime-monitor.py tactical-response
```
...
```bash
printf '%s\n' '{"ok":true,"mode":"alert","alert":true,"reasons":[{"kind":"hostile_creep","room":"shardX/E29N55","object_id":"hostile-1","owner":"Invader","x":20,"y":21,"message":"hostile creep visible: Invader at 20,21"}],"rooms":["shardX/E29N55"]}' \
  | python3 scripts/screeps-runtime-monitor.py tactical-response
```
...
```bash
# Load SCREEPS_AUTH_TOKEN into the environment without printing it.
python3 scripts/screeps-runtime-monitor.py summary --room shardX/E29N55 --out-dir /root/screeps/runtime-artifacts/screeps-monitor-smoke
SCREEPS_MONITOR_STATE_FILE=/tmp/screeps-monitor-alert-smoke-state.json \
