...
## Roadmap domain next-point progress

These percentages are explicit next-point estimates for the current roadmap snapshot; they should move only when there is verified evidence such as merged PRs, passing checks, redacted live reports, rendered artifacts, scheduler-health proof, or accepted gameplay-review conclusions.

| Domain | Next-point completion | Current next point |
| --- | ---: | --- |
| Agent OS / visibility | 100% | current P0 scheduler/visibility incident repaired in `docs/process/2026-04-26-p0-repair-completion-2320.md`; continue routine P0 monitor watch and reopen/escalate if a future job shows stale `next_run_at`, missing finalized output, delivery errors, or workdir serialization risk |
...
| Gameplay Evolution | 10% | source-backed KPI framework and GitHub-tracked 12h review/task bridge专项 started in `docs/ops/gameplay-evolution-roadmap.md` |
...
Status: pinned Dockerized path passed room/map initialization and bot tick validation; PR #16 live-rerun fixes made the committed harness pass a fresh live run from an ignored workdir, and PR #16 has merged to `main`.
...
- Local secret/config presence now includes `SCREEPS_AUTH_TOKEN` and `STEAM_KEY` without printing secret values; current safe selectors are `SCREEPS_BRANCH=main`, `SCREEPS_API_URL=https://screeps.com`, `SCREEPS_SHARD=shardX`, and `SCREEPS_ROOM=E29N55`.
...
- Remaining work: wire scheduled runtime-summary/runtime-alert monitoring using the now-repeat-smoked live-token monitor path. Basic room initialization and harness live execution are no longer blockers.
...
### Decision: Gameplay Evolution专项 becomes an explicit roadmap track
...
- The roadmap must be driven by an evidence loop: actual game result → KPI review → bottleneck diagnosis → GitHub issue/task update → Codex implementation → gated release → next observation window.
- Priority order is now explicit: P0 automation-system blockers first; then direct game-goal delivery in the vision order territory, resources, enemy kills; then non-blocking foundation work. Foundation work may outrank a gameplay item only when it is already blocking that gameplay item's evidence, implementation, release, or rollback path.
- Scheduled 12-hour gameplay reviews should evaluate territory, resources, enemy/combat outcomes, and reliability guardrails.
- Six-hour reports and roadmap snapshots must foreground game KPI movement plus the delivery state of corresponding build items, not mostly generic process progress.
- Tactical emergency response must be separate from the 12-hour cadence so attacks, spawn collapse, or room-loss risks can trigger bounded triage and hotfix work immediately.
- Production code tasks generated by reviews still use the existing Codex + PR + QA acceptance pipeline.
- Worker allocation must be parallel across tracks: gameplay development and foundation/operations both need active or queued work when unblocked, with more capacity assigned to the higher-priority game-goal bottleneck.
