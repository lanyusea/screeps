...
- Harness status: PR #12 `scripts/screeps-private-smoke.py` supports offline `self-test`, `dry-run`, and live `run`; review hardening now rejects unsafe repo-local workdirs before writing secrets, requires a stable password when reusing no-reset server data, retries transient `/stats` failures until the deadline, requires room-specific Mongo spawn evidence for the `already playing` placement path, bounds Mongo summaries so JSON remains parseable, pins Docker image tags (`screepers/screeps-launcher:v1.16.2`, `mongo:8.2.7`, `redis:7.4.8`), signs in with `cfg.email`, and adds docstrings/self-test coverage for CodeRabbit feedback. Latest Codex commit: `cad16f9` (`fix: pin smoke harness images and signin email`).
...
- Latest verification:
  ...
  - Private smoke harness self-test: `python3 scripts/screeps-private-smoke.py self-test` passed, 13 tests after PR #12 review fix `cad16f9`
