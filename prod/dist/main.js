...
function withdrawFromStorage(room, amount, storage = getRoomStorage(room), currentStorageEnergy = storage ? getStoredEnergy2(storage) : 0) {
  if (!storage) {
    return false;
  }
  return currentStorageEnergy - normalizeEnergyAmount(amount) >= getEffectiveRoomEnergyBufferThreshold(room);
}
function getStorageEnergyAvailableForWithdrawal(room, storage = getRoomStorage(room), currentStorageEnergy = storage ? getStoredEnergy2(storage) : 0) {
  if (!storage) {
    return 0;
  }
...
function selectStorageToSpawnExtensionRefillAcquisitionTask(creep) {
  ...
  if (plannedWithdrawal <= 0 || plannedWithdrawal > getStorageEnergyAvailableForWithdrawal(creep.room, storage, projectedStorageEnergy) || !withdrawFromStorage(creep.room, plannedWithdrawal, storage, projectedStorageEnergy)) {
    return null;
  }
...
function findSpawnExtensionRefillTargets(room) {
  const structures = findOwnedStructures3(room).filter(
    (structure) => (matchesStructureType12(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType12(structure.structureType, "STRUCTURE_EXTENSION", "extension")) && getFreeEnergyCapacity5(structure) > 0
  );
...
function findTowerRefillTargets(room) {
  return findOwnedStructures3(room).filter(
    (structure) => matchesStructureType12(structure.structureType, "STRUCTURE_TOWER", "tower") && getStoredEnergy9(structure) < TOWER_REFILL_THRESHOLD && getFreeEnergyCapacity5(structure) > 0
  ).map((target) => ({
    freeCapacity: getFreeEnergyCapacity5(target),
    id: getObjectId5(target),
    priority: 2,
