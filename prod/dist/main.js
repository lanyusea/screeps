...
function buildDefenseTelemetryContext(room) {
...
function getCriticalStructures(room) {
  var _a;
...
function isCriticalStructure(structure) {
  return matchesStructureType(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType(structure.structureType, "STRUCTURE_CONTROLLER", "controller") || matchesStructureType(structure.structureType, "STRUCTURE_TOWER", "tower") || matchesStructureType(structure.structureType, "STRUCTURE_RAMPART", "rampart");
}
function isDamagedStructure(structure) {
  return typeof structure.hits === "number" && typeof structure.hitsMax === "number" && structure.hitsMax > 0 && structure.hits < structure.hitsMax * CRITICAL_STRUCTURE_DAMAGE_RATIO;
}
...
function refreshVisibleDeadZoneMemory(gameTime = getGameTime3()) {
  var _a;
...
function refreshVisibleRoomDeadZoneMemory(room, gameTime = getGameTime3()) {
  var _a;
...
function isDeadZoneMemoryExpired(roomMemory, gameTime = getGameTime3()) {
...
// src/defense/safeModeManager.ts
var SAFE_MODE_HOSTILE_COUNT_THRESHOLD = 2;
var CRITICAL_SPAWN_LOSS_HITS_RATIO = 0.25;
var OK_CODE = 0;
function runSafeModeWithResult(room) {
  var _a;
  const context = buildDefenseTelemetryContext(room);
  const events = [];
  const result = { events, activated: false };
  if (!shouldActivateSafeMode(context)) {
    return result;
  }
  const controller = context.room.controller;
  const activationResult = (_a = controller == null ? void 0 : controller.activateSafeMode) == null ? void 0 : _a.call(controller);
...
function shouldActivateSafeMode(context) {
  const controller = context.room.controller;
  if (context.hostileCreeps.length === 0 || (controller == null ? void 0 : controller.my) !== true || typeof controller.activateSafeMode !== "function" || !isSafeModeAvailable(controller)) {
    return false;
  }
  if (isCriticalSpawnLossThreat(context)) {
    return true;
  }
  return context.hostileCreeps.length > SAFE_MODE_HOSTILE_COUNT_THRESHOLD && isControllerUnderAttack(controller, context.hostileCreeps);
}
...
function isControllerUnderAttack(controller, hostileCreeps) {
  ...
  return hostileCreeps.some((hostile) => {
    ...
    const range = (_b = (_a = controller.pos).getRangeTo) == null ? void 0 : _b.call(_a, hostile.pos);
    return typeof range !== "number" || range <= 3;
  });
}
function isCriticalSpawnLossThreat(context) {
...
function isOwnedSpawn(structure) {
  var _a;
...
function isCriticallyDamagedSpawn(spawn) {
  return typeof spawn.hits === "number" && typeof spawn.hitsMax === "number" && spawn.hitsMax > 0 && spawn.hits < spawn.hitsMax * CRITICAL_SPAWN_LOSS_HITS_RATIO;
}

// src/defense/towerManager.ts
var TOWER_RECOVERY_ENERGY_RESERVE = 250;
var OK_CODE2 = 0;
function runTowersWithResult(room) {
  const context = buildDefenseTelemetryContext(room);
  const events = [];
  const result = {
    events,
    ...
    actedTowerIds: /* @__PURE__ */ new Set()
  };
...
function runTowerAttack(tower, context, result) {
  var _a;
  if (typeof tower.attack !== "function") {
    return false;
  }
  const target = (_a = selectClosestTarget(tower, context.hostileCreeps)) != null ? _a : selectClosestTarget(tower, context.hostileStructures);
  if (!target) {
    return false;
  }
...
function runTowerRepair(tower, context, result) {
  ...
  result.actedTowerIds.add(getObjectId(tower));
  return repairResult === OK_CODE2;
}
function hasHostileTarget(context) {
  return context.hostileCreeps.length > 0 || context.hostileStructures.length > 0;
}
function getUsableTowers(room) {
...
function isTargetInTowerRoom(origin, target) {
...
var MAX_RECORDED_DEFENSE_ACTIONS2 = 20;
  ...
  recordDefenseAction2(
    {
      ...
      targetId: getObjectId2(target)
    },
    telemetryEvents
  );
}
function createDefenseContext(colony) {
  const criticalStructures = getCriticalStructures2(colony);
  return {
    colony,
    damagedCriticalStructures: criticalStructures.filter(isDamagedStructure),
    hostileCreeps: findHostileCreeps2(colony.room),
    hostileStructures: findHostileStructures2(colony.room)
  };
}
...
function getCriticalStructures2(colony) {
  const structuresById = /* @__PURE__ */ new Map();
  for (const spawn of colony.spawns) {
    structuresById.set(getObjectId2(spawn), spawn);
  }
  for (const tower of getOwnedTowers2(colony.room)) {
    structuresById.set(getObjectId2(tower), tower);
  }
  return [...structuresById.values()].sort(compareObjectIds2);
}
function getOwnedTowers2(room) {
  return findOwnedStructures2(room).filter(
    (structure) => matchesStructureType2(structure.structureType, "STRUCTURE_TOWER", "tower")
  );
}
function selectDefenderTarget(creep) {
  const hostileCreep = selectClosestTarget2(creep, findHostileCreeps2(creep.room));
  if (hostileCreep) {
    return hostileCreep;
  }
  return selectClosestTarget2(creep, findHostileStructures2(creep.room));
}
function selectClosestTarget2(origin, targets) {
  if (targets.length === 0) {
    return null;
  }
  return [...targets].sort((left, right) => compareRange2(origin, left, right) || compareObjectIds2(left, right))[0];
}
...
function recordDefenseAction2(input, telemetryEvents) {
  const actionMemory = {
    type: input.action,
  ...
  recordDefenseActionMemory2(actionMemory);
  telemetryEvents.push({
    type: "defense",
    action: actionMemory.type,
    roomName: actionMemory.roomName,
...
function recordDefenseActionMemory2(action) {
  var _a, _b, _c;
  const globalMemory = globalThis.Memory;
  if (!globalMemory) {
    return;
  ...
  rooms[action.roomName] = action;
  defenseMemory.rooms = rooms;
  defenseMemory.actions = [action, ...(_c = defenseMemory.actions) != null ? _c : []].slice(0, MAX_RECORDED_DEFENSE_ACTIONS2);
  globalMemory.defense = defenseMemory;
}
function findHostileCreeps2(room) {
  return findRoomObjects3(room, "FIND_HOSTILE_CREEPS");
}
function findHostileStructures2(room) {
  return findRoomObjects3(room, "FIND_HOSTILE_STRUCTURES");
}
function findOwnedStructures2(room) {
  return findRoomObjects3(room, "FIND_MY_STRUCTURES");
}
...
function matchesStructureType2(value, globalName, fallback) {
  var _a;
  const expectedValue = (_a = globalThis[globalName]) != null ? _a : fallback;
  return value === expectedValue;
}
...
function getCreepName(creep) {
  return typeof creep.name === "string" ? creep.name : getObjectId2(creep);
}
...
function recordColonySurvivalAssessment(colonyName, assessment, tick = getGameTime5()) {
  if (!isNonEmptyString2(colonyName) || tick === null) {
    return;
  }
  stageAssessmentByColony.set(colonyName, { assessment, tick });
}
function persistColonyStageAssessment(colony, assessment, tick = getGameTime5()) {
  if (tick === null) {
    return;
  }
...
function getRecordedColonySurvivalAssessment(colonyName, tick = getGameTime5()) {
  if (!isNonEmptyString2(colonyName) || tick === null) {
    return null;
  }
...
function getRecordedColonyStageAssessment(colonyName, tick = getGameTime5()) {
...
function findSources(room) {
  ...
  const sourceFindConstant = getGlobalNumber3("FIND_SOURCES");
...
function findLogisticsTargetPositions(room) {
  var _a;
  const sourcePositions = findRoomObjects4(room, "FIND_SOURCES").map((source) => source.pos).filter((position) => isSameRoomPosition(position, room.name));
...
function isRoadWorkTarget(target) {
  return matchesStructureType3(target.structureType, "STRUCTURE_ROAD", "road");
}
...
function isRoadStructure(structure) {
  return matchesStructureType4(structure.structureType, "STRUCTURE_ROAD", "road");
}
function isRoadConstructionSite(site) {
  return matchesStructureType4(site.structureType, "STRUCTURE_ROAD", "road");
}
...
function countExistingAndPendingFixedStructures(room, globalName, fallback) {
  ...
  return ownedStructures.filter((structure) => matchesStructureType5(structure.structureType, globalName, fallback)).length + ownedConstructionSites.filter((site) => matchesStructureType5(String(site.structureType), globalName, fallback)).length;
}
...
function persistOccupationRecommendationFollowUpIntent(report, gameTime = getGameTime6()) {
  var _a, _b;
...
function enrichVisibleOccupationCandidate(candidate) {
  ...
  const hostileCreeps = findRoomObjects6(room, "FIND_HOSTILE_CREEPS");
  const hostileStructures = findRoomObjects6(room, "FIND_HOSTILE_STRUCTURES");
  const sources = findRoomObjects6(room, "FIND_SOURCES");
  const constructionSites = findRoomObjects6(room, "FIND_MY_CONSTRUCTION_SITES");
  const ownedStructures = findRoomObjects6(room, "FIND_MY_STRUCTURES");
...
function hasOccupationRoomEvidence(candidate) {
  return candidate.visible || candidate.scouted === true;
}
function getOccupationRoomEvidenceLabel(candidate) {
  return candidate.visible ? "room visible" : "room scouted";
}
function getOccupationEvidenceAdjective(candidate) {
...
function getVisibleController(targetRoom, controllerId) {
  ...
  const getObjectById4 = game == null ? void 0 : game.getObjectById;
  if (controllerId && typeof getObjectById4 === "function") {
    return getObjectById4.call(game, controllerId);
  }
  return null;
}
...
function recordVisibleRoomScoutIntel(colony, room, gameTime = getGameTime7(), scoutName, telemetryEvents = []) {
  var _a, _b, _c, _d;
  if (!isNonEmptyString6(colony) || !room || !isNonEmptyString6(room.name)) {
    return null;
  }
...
function ensureTerritoryScoutAttempt(colony, targetRoom, gameTime, telemetryEvents = [], controllerId) {
  var _a;
  if (!isNonEmptyString6(colony) || !isNonEmptyString6(targetRoom)) {
    return null;
  }
  const territoryMemory = getWritableTerritoryMemoryRecord2();
  if (!territoryMemory) {
...
function validateTerritoryScoutIntelForClaim({
  ...
  if (controller.my === true || isNonEmptyString6(controller.ownerUsername) && controller.ownerUsername === colonyOwnerUsername) {
...
function buildTerritoryScoutIntel(colony, room, gameTime, scoutName) {
  ...
  return {
    ...
    ...sourceAccessPoints !== void 0 ? { sourceAccessPoints } : {},
    ...controllerSourceRange !== void 0 ? { controllerSourceRange } : {},
    ...terrain ? { terrain } : {},
...
function summarizeScoutSources(sources, terrain) {
  return sources.flatMap((source) => {
    if (!source.pos || !isFiniteNumber5(source.pos.x) || !isFiniteNumber5(source.pos.y)) {
      return [];
    }
    ...
    return [
      {
        ...
        ...accessPoints !== void 0 ? { accessPoints } : {}
      }
    ];
  });
}
function calculateSourceAccessPoints(pos, terrain) {
...
function calculateAverageSourceAccessPoints(sources) {
  const accessPoints = sources.flatMap(
    (source) => isFiniteNumber5(source.accessPoints) ? [source.accessPoints] : []
  );
  if (accessPoints.length === 0) {
    return void 0;
  }
  const average2 = accessPoints.reduce((total, value) => total + value, 0) / accessPoints.length;
...
function calculateAverageControllerSourceRange(controller, sources) {
  ...
  const ranges = sources.map(
    (source) => Math.max(Math.abs(controller.pos.x - source.x), Math.abs(controller.pos.y - source.y))
  );
...
function getTerritoryScoutIntel(colony, targetRoom) {
  var _a, _b;
  const rawIntel = (_b = (_a = getTerritoryMemoryRecord3()) == null ? void 0 : _a.scoutIntel) == null ? void 0 : _b[getTerritoryScoutMemoryKey(colony, targetRoom)];
  return normalizeTerritoryScoutIntel(rawIntel);
}
function getTerritoryScoutAttempt(colony, targetRoom) {
  var _a, _b;
  const rawAttempt = (_b = (_a = getTerritoryMemoryRecord3()) == null ? void 0 : _a.scoutAttempts) == null ? void 0 : _b[getTerritoryScoutMemoryKey(colony, targetRoom)];
...
function getMutableScoutAttemptRecords(territoryMemory) {
  if (!isRecord6(territoryMemory.scoutAttempts) || Array.isArray(territoryMemory.scoutAttempts)) {
    territoryMemory.scoutAttempts = {};
  }
  return territoryMemory.scoutAttempts;
}
function getMutableScoutIntelRecords(territoryMemory) {
  if (!isRecord6(territoryMemory.scoutIntel) || Array.isArray(territoryMemory.scoutIntel)) {
    territoryMemory.scoutIntel = {};
  }
  return territoryMemory.scoutIntel;
}
function normalizeTerritoryScoutAttempt(rawAttempt) {
  if (!isRecord6(rawAttempt)) {
    return null;
  }
  if (!isNonEmptyString6(rawAttempt.colony) || !isNonEmptyString6(rawAttempt.roomName) || !isTerritoryScoutAttemptStatus(rawAttempt.status) || !isFiniteNumber5(rawAttempt.requestedAt) || !isFiniteNumber5(rawAttempt.updatedAt)) {
    return null;
  }
  const attemptCount = isFiniteNumber5(rawAttempt.attemptCount) ? Math.max(1, Math.floor(rawAttempt.attemptCount)) : 1;
  ...
  return {
    ...
    ...isNonEmptyString6(rawAttempt.scoutName) ? { scoutName: rawAttempt.scoutName } : {},
    ...lastValidation ? { lastValidation } : {}
  };
}
function normalizeTerritoryScoutIntel(rawIntel) {
  if (!isRecord6(rawIntel)) {
    return null;
  }
  if (!isNonEmptyString6(rawIntel.colony) || !isNonEmptyString6(rawIntel.roomName) || !isFiniteNumber5(rawIntel.updatedAt)) {
    return null;
  }
  const sourceIds = Array.isArray(rawIntel.sourceIds) ? rawIntel.sourceIds.flatMap((sourceId) => isNonEmptyString6(sourceId) ? [sourceId] : []) : [];
  const sourceCount = isFiniteNumber5(rawIntel.sourceCount) ? Math.max(0, Math.floor(rawIntel.sourceCount)) : sourceIds.length;
  ...
  return {
    ...
    sourceCount,
    ...sourcePositions.length > 0 ? { sourcePositions } : {},
    ...isFiniteNumber5(rawIntel.sourceAccessPoints) ? { sourceAccessPoints: Math.max(0, rawIntel.sourceAccessPoints) } : {},
    ...isFiniteNumber5(rawIntel.controllerSourceRange) ? { controllerSourceRange: Math.max(0, Math.floor(rawIntel.controllerSourceRange)) } : {},
    ...terrain ? { terrain } : {},
    ...mineral ? { mineral } : {},
    ...
    ...isNonEmptyString6(rawIntel.scoutName) ? { scoutName: rawIntel.scoutName } : {}
  };
}
function normalizeTerritoryScoutSourceIntel(rawSources) {
  if (!Array.isArray(rawSources)) {
    return [];
  }
  return rawSources.flatMap((rawSource) => {
    if (!isRecord6(rawSource) || !isNonEmptyString6(rawSource.id)) {
      return [];
    }
    if (!isFiniteNumber5(rawSource.x) || !isFiniteNumber5(rawSource.y)) {
      return [];
    }
    return [
      {
        ...
        ...isFiniteNumber5(rawSource.accessPoints) ? { accessPoints: Math.max(0, Math.floor(rawSource.accessPoints)) } : {}
      }
    ];
  });
}
function normalizeTerritoryScoutControllerIntel(rawController) {
  if (!isRecord6(rawController)) {
    return null;
  }
  return {
    ...typeof rawController.id === "string" ? { id: rawController.id } : {},
    ...typeof rawController.my === "boolean" ? { my: rawController.my } : {},
    ...isNonEmptyString6(rawController.ownerUsername) ? { ownerUsername: rawController.ownerUsername } : {},
    ...isNonEmptyString6(rawController.reservationUsername) ? { reservationUsername: rawController.reservationUsername } : {},
    ...isFiniteNumber5(rawController.reservationTicksToEnd) ? { reservationTicksToEnd: rawController.reservationTicksToEnd } : {}
  };
}
function normalizeTerritoryScoutMineralIntel(rawMineral) {
  if (!isRecord6(rawMineral) || !isNonEmptyString6(rawMineral.id)) {
    return null;
  }
  return {
    id: rawMineral.id,
    ...isNonEmptyString6(rawMineral.mineralType) ? { mineralType: rawMineral.mineralType } : {},
    ...isFiniteNumber5(rawMineral.density) ? { density: rawMineral.density } : {}
  };
}
function normalizeTerritoryTerrainQuality(rawTerrain) {
  if (!isRecord6(rawTerrain)) {
    return null;
  }
  if (!isFiniteNumber5(rawTerrain.walkableRatio) || !isFiniteNumber5(rawTerrain.swampRatio) || !isFiniteNumber5(rawTerrain.wallRatio)) {
    return null;
  }
  return {
...
function normalizeTerritoryScoutValidation(rawValidation) {
  if (!isRecord6(rawValidation)) {
    return null;
  }
  if (!isTerritoryScoutValidationStatus(rawValidation.status) || !isFiniteNumber5(rawValidation.updatedAt)) {
    return null;
  }
  return {
...
function getBoundedCount(value) {
  return isFiniteNumber5(value) ? Math.max(0, Math.floor(value)) : 0;
}
...
function recordRecoveredTerritoryFollowUpRetryCooldown(plan, gameTime = getGameTime8()) {
...
function shouldSpawnTerritoryControllerCreep(plan, roleCounts, gameTime = getGameTime8()) {
...
function getTerritoryFollowUpPreparationWorkerDemand(plan, gameTime = getGameTime8()) {
  var _a;
...
function hasActiveTerritoryFollowUpPreparationDemand(colony, gameTime = getGameTime8()) {
...
function hasPendingTerritoryFollowUpIntent(colony, roleCounts, gameTime = getGameTime8()) {
  if (!isNonEmptyString7(colony)) {
    return false;
  }
  const territoryMemory = getTerritoryMemoryRecord4();
...
function getActiveTerritoryFollowUpExecutionHints(colony = void 0) {
  ...
  return getBoundedActiveTerritoryFollowUpExecutionHints(
    ...
    intents
  ).filter((hint) => !isNonEmptyString7(colony) || hint.colony === colony);
}
...
function getSuspendedTerritoryIntentCountsByRoom(colony, gameTime = getGameTime8()) {
  var _a;
  if (!isNonEmptyString7(colony)) {
    return {};
  }
...
function canCreepReserveTerritoryController(creep, controller, colony) {
  ...
  if (!isNonEmptyString7(actorUsername) || !isNonEmptyString7(reservation.username) || reservation.username !== actorUsername || typeof reservation.ticksToEnd !== "number") {
    return false;
  }
...
function isVisibleTerritoryAssignmentSafe(assignment, colony, creep) {
  if (!isNonEmptyString7(assignment.targetRoom)) {
    return false;
  }
  ...
  if (isNonEmptyString7(colony) && isTerritoryIntentSuppressed(colony, assignment.targetRoom, assignment.action)) {
    return false;
  }
...
function isVisibleTerritoryAssignmentComplete(assignment, creep) {
  if (assignment.action !== "claim" || !isNonEmptyString7(assignment.targetRoom)) {
    return false;
  }
  const controller = selectVisibleTerritoryAssignmentController(assignment, creep);
...
function suppressTerritoryIntent(colony, assignment, gameTime) {
  if (!isNonEmptyString7(colony) || !isNonEmptyString7(assignment.targetRoom) || !isTerritoryIntentAction2(assignment.action)) {
    return;
  }
  const territoryMemory = getWritableTerritoryMemoryRecord3();
  if (!territoryMemory) {
...
function recordTerritoryReserveFallbackIntent(colony, assignment, gameTime) {
  if (!isNonEmptyString7(colony) || !isNonEmptyString7(assignment.targetRoom) || assignment.action !== "reserve" || isTerritoryIntentSuppressed(colony, assignment.targetRoom, "reserve", gameTime)) {
    return null;
  }
  const territoryMemory = getWritableTerritoryMemoryRecord3();
  if (!territoryMemory) {
...
function recordAutonomousExpansionClaimReserveFallbackIntent(colony, evaluation, gameTime) {
  if (evaluation.status !== "skipped" || evaluation.reason !== "gclInsufficient" && evaluation.reason !== "controllerCooldown" || !isNonEmptyString7(colony) || !isNonEmptyString7(evaluation.targetRoom) || isTerritoryIntentSuppressed(colony, evaluation.targetRoom, "reserve", gameTime)) {
    return null;
  }
  const targetRoom = evaluation.targetRoom;
  if (!isNonEmptyString7(targetRoom)) {
    return null;
  }
  const territoryMemory = getWritableTerritoryMemoryRecord3();
  if (!territoryMemory) {
  ...
  if (!isNonEmptyString7(colonyOwnerUsername)) {
    return null;
  }
...
function refreshRemoteMiningSetup(colony, gameTime = getGameTime8()) {
  var _a, _b, _c, _d;
  const territoryMemory = getWritableTerritoryMemoryRecord3();
  if (!territoryMemory) {
    return;
  ...
  let remoteMining;
  if (isRecord7(storedRemoteMining) && !Array.isArray(storedRemoteMining)) {
...
function getConfiguredTerritoryCandidateAction(target, colonyOwnerUsername, territoryMemory, gameTime) {
  if (target.action !== "reserve" || !isNonEmptyString7(colonyOwnerUsername)) {
...
function getStoredTerritoryReservation(territoryMemory, target) {
  ...
  const reservation = normalizeTerritoryReservation(
    isRecord7(territoryMemory.reservations) ? territoryMemory.reservations[getTerritoryReservationMemoryKey(target.colony, target.roomName)] : void 0
  );
...
function normalizeTerritoryReservation(rawReservation) {
  if (!isRecord7(rawReservation)) {
    return null;
  }
  if (!isNonEmptyString7(rawReservation.colony) || !isNonEmptyString7(rawReservation.roomName) || !isFiniteNumber6(rawReservation.ticksToEnd) || !isFiniteNumber6(rawReservation.updatedAt)) {
    return null;
  }
  return {
...
function hasMalformedTerritoryReservationMemory(rawReservations, reservations) {
  return isRecord7(rawReservations) && Object.keys(rawReservations).length !== Object.keys(reservations).length;
}
...
function getPersistedExpansionCandidateRanks(colonyName, territoryMemory) {
  ...
  for (const [index, rawCandidate] of territoryMemory.expansionCandidates.entries()) {
    ...
    ranks.set(
      ...
      isFiniteNumber6(rawCandidate.rank) && rawCandidate.rank > 0 ? rawCandidate.rank : index + 1
    );
  }
  return ranks;
}
...
function isScoutedAdjacentControlCandidate(candidate) {
  return candidate.source === "adjacent" && candidate.intentAction === "scout" && isTerritoryControlAction3(candidate.target.action);
}
...
function getOccupationIntentActionableTicks(selection, colonyOwnerUsername) {
  ...
  if (selection.intentAction === "reserve") {
    ...
    return (_a = ownReservationTicksToEnd != null ? ownReservationTicksToEnd : getControllerReservationTicksToEnd2(controller)) != null ? _a : 0;
  }
  if (isControllerOwned(controller)) {
    return typeof controller.ticksToDowngrade === "number" ? controller.ticksToDowngrade : void 0;
  }
  return (_b = getControllerReservationTicksToEnd2(controller)) != null ? _b : 0;
}
...
function buildTerritoryFollowUp(source, originRoom) {
  ...
  if (originAction === null || !isTerritoryFollowUpSource2(source) || !isNonEmptyString7(originRoom)) {
    return {};
  }
  return {
    followUp: {
...
function getVisibleOwnedRoomNames2(fallbackRoomName) {
  ...
  for (const room of Object.values(rooms)) {
    if (((_b = room == null ? void 0 : room.controller) == null ? void 0 : _b.my) === true && isNonEmptyString7(room.name)) {
      roomNames.add(room.name);
    }
  }
  return Array.from(roomNames);
...
function getTerritoryRouteDistanceCache() {
  ...
  if (!isRecord7(territoryMemory.routeDistances)) {
...
function normalizeTerritoryTarget2(rawTarget) {
  if (!isRecord7(rawTarget)) {
    return null;
  }
  if (!isNonEmptyString7(rawTarget.colony) || !isNonEmptyString7(rawTarget.roomName) || !isTerritoryControlAction3(rawTarget.action)) {
    return null;
  }
  return {
    colony: rawTarget.colony,
...
function isActiveTerritoryFollowUpIntent(intent) {
  return (intent.status === "planned" || intent.status === "active") && intent.followUp !== void 0 && !isTerritoryIntentSuspensionActive(intent, getGameTime8());
}
...
function normalizeTerritoryFollowUpExecutionHint(rawHint) {
  if (!isRecord7(rawHint)) {
    return null;
  }
  if (rawHint.type !== "activeFollowUpExecution" || !isNonEmptyString7(rawHint.colony) || !isNonEmptyString7(rawHint.targetRoom) || !isTerritoryIntentAction2(rawHint.action) || !isTerritoryExecutionHintReason(rawHint.reason) || typeof rawHint.updatedAt !== "number") {
    return null;
  }
...
function normalizeTerritoryFollowUpDemand(rawDemand) {
  if (!isRecord7(rawDemand)) {
    return null;
  }
  if (rawDemand.type !== "followUpPreparation" || !isNonEmptyString7(rawDemand.colony) || !isNonEmptyString7(rawDemand.targetRoom) || !isTerritoryControlAction3(rawDemand.action) || typeof rawDemand.updatedAt !== "number") {
    return null;
  }
...
function isTerritoryIntentSuspended(colony, targetRoom, action, gameTime = getGameTime8()) {
  const territoryMemory = getTerritoryMemoryRecord4();
  if (!territoryMemory) {
    return false;
  }
...
function isTerritoryIntentSuppressed(colony, targetRoom, action, gameTime = getGameTime8()) {
  const territoryMemory = getTerritoryMemoryRecord4();
  if (!territoryMemory) {
    return false;
  }
...
function isRecoveredTerritoryFollowUpAttemptCoolingDown2(intent, gameTime) {
  return intent.followUp !== void 0 && isFiniteNumber6(intent.lastAttemptAt) && gameTime >= intent.lastAttemptAt && gameTime - intent.lastAttemptAt <= TERRITORY_RECOVERED_FOLLOW_UP_RETRY_COOLDOWN_TICKS2;
}
...
function selectVisibleTerritoryControllerIntent(creep) {
  ...
  if (!isNonEmptyString7(roomName) || isVisibleRoomUnsafe(creep.room)) {
    return null;
  }
...
function normalizeCreepTerritoryIntent(creep, roomName) {
  ...
  if (!assignment || assignment.targetRoom !== roomName || !isTerritoryControlAction3(assignment.action) || isNonEmptyString7((_b = creep.memory) == null ? void 0 : _b.colony) && isTerritoryIntentSuppressed(creep.memory.colony, assignment.targetRoom, assignment.action)) {
    return null;
  }
  const followUp = normalizeTerritoryFollowUp(assignment.followUp);
  return {
...
function isActiveVisibleControllerIntentForCreep(intent, roomName, creepColony) {
  return intent.targetRoom === roomName && intent.targetRoom !== intent.colony && isTerritoryControlAction3(intent.action) && (intent.status === "planned" || intent.status === "active") && (!isNonEmptyString7(creepColony) || intent.colony === creepColony);
}
...
function selectCreepRoomController(creep, controllerId) {
  ...
  const game = globalThis.Game;
  const getObjectById4 = game == null ? void 0 : game.getObjectById;
  if (typeof getObjectById4 !== "function") {
    return null;
  }
  return getObjectById4.call(game, controllerId);
}
...
function getTerritoryActorUsername(creep, colony) {
  var _a;
  return (_a = getCreepOwnerUsername(creep)) != null ? _a : isNonEmptyString7(colony) ? getVisibleColonyOwnerUsername2(colony) : null;
}
function getCreepOwnerUsername(creep) {
  var _a;
  const username = (_a = creep == null ? void 0 : creep.owner) == null ? void 0 : _a.username;
  return isNonEmptyString7(username) ? username : null;
}
...
function isControllerOwnedByColony2(controller, colonyOwnerUsername) {
  const ownerUsername = getControllerOwnerUsername3(controller);
  return controller.my === true || isNonEmptyString7(ownerUsername) && ownerUsername === colonyOwnerUsername;
}
function getReserveControllerTargetState(controller, colonyOwnerUsername) {
  ...
  if (!isNonEmptyString7(reservation.username) || reservation.username !== colonyOwnerUsername) {
    return "unavailable";
  }
...
function isForeignReservedController(controller, actorUsername) {
  if (isControllerOwned(controller) || !isNonEmptyString7(actorUsername)) {
    return false;
  }
  const reservation = controller.reservation;
  return isNonEmptyString7(reservation == null ? void 0 : reservation.username) && reservation.username !== actorUsername;
}
function isOwnReservedController(controller, actorUsername) {
  const reservation = controller.reservation;
  return isNonEmptyString7(actorUsername) && isNonEmptyString7(reservation == null ? void 0 : reservation.username) && reservation.username === actorUsername;
}
...
function getOwnReservationTicksToEnd(controller, colonyOwnerUsername) {
  if (isControllerOwned(controller) || !isNonEmptyString7(colonyOwnerUsername)) {
    return null;
  }
  const reservation = controller.reservation;
...
function getVisibleColonyOwnerUsername2(colonyName) {
  ...
  return getControllerOwnerUsername3(controller != null ? controller : void 0);
}
...
function isRemoteMiningBootstrapRecord(record, colonyName) {
  return isRecord7(record) && record.colony === colonyName && isNonEmptyString7(record.roomName) && record.roomName !== colonyName && isPostClaimRemoteMiningStatus(record.status);
}
...
function getTerritoryMemoryRecord4() {
  const memory = getMemoryRecord();
  if (!memory || !isRecord7(memory.territory)) {
    return null;
  }
  return memory.territory;
}
...
function findRoomStructures(room) {
  ...
  for (const structure of [
    ...findRoomObjects8(room, "FIND_MY_STRUCTURES"),
    ...findRoomObjects8(room, "FIND_STRUCTURES")
  ]) {
    const stableId = getObjectId3(structure);
...
function findRoomObjects8(room, globalName) {
...
function canLinkSendEnergy(link, projectedState) {
  var _a;
  return getLinkCooldown(link) <= 0 && ((_a = projectedState.storedEnergyById.get(getObjectId4(link))) != null ? _a : 0) > 0;
}
...
function selectSourceLinks(room, links, destinationIds) {
  ...
  return links.filter((link) => !destinationIds.has(getObjectId4(link))).filter((link) => sources.some((source) => isNearRoomObject2(link, source, room.name, SOURCE_LINK_RANGE))).sort(compareObjectIds3);
}
...
function selectStorageLink(room, links, controllerLink) {
  ...
  return selectClosestLink(
    links.filter((link) => getObjectId4(link) !== getObjectId4(controllerLink)),
    storage,
    room.name,
    STORAGE_LINK_RANGE
  );
...
function selectClosestLink(links, target, roomName, range) {
  ...
  return (_a = links.filter((link) => isNearRoomObject2(link, target, roomName, range)).sort(
    (left, right) => compareRangeToPosition(targetPosition, left, right) || getObjectId4(left).localeCompare(getObjectId4(right))
  )[0]) != null ? _a : null;
}
...
function findOwnedLinks(room) {
  return findOwnedStructures3(room).filter((structure) => matchesStructureType8(structure.structureType, "STRUCTURE_LINK", "link")).sort(compareObjectIds3);
}
...
function findStorage(room) {
  ...
  return (_a = findOwnedStructures3(room).filter(
    (structure) => matchesStructureType8(structure.structureType, "STRUCTURE_STORAGE", "storage")
  )[0]) != null ? _a : null;
}
...
function compareObjectIds3(left, right) {
  return getObjectId4(left).localeCompare(getObjectId4(right));
}
...
function buildWorkerTaskBehaviorState(creep) {
  ...
  const structures = findRoomObjects9(room, getFindConstant3("FIND_STRUCTURES"));
  const myStructures = findRoomObjects9(room, getFindConstant3("FIND_MY_STRUCTURES"));
  const constructionSites = findRoomObjects9(room, getFindConstant3("FIND_CONSTRUCTION_SITES"));
  const droppedResources = findRoomObjects9(room, getFindConstant3("FIND_DROPPED_RESOURCES"));
  const sources = findRoomObjects9(room, getFindConstant3("FIND_SOURCES"));
  const hostileCreeps = findRoomObjects9(room, getFindConstant3("FIND_HOSTILE_CREEPS"));
  ...
  return {
    ...
    freeCapacity,
    energyCapacity,
    energyLoadRatio: roundRatio2(carriedEnergy, energyCapacity),
    currentTask,
    ...
    roadCoverage: roundRatio2(nearbyRoadCount, NEARBY_TILE_COUNT),
    hostileCreepCount: hostileCreeps.length,
...
function buildControllerState(controller) {
  ...
  return {
    ...
    ...progress !== void 0 && progressTotal !== void 0 && progressTotal > 0 ? { controllerProgressRatio: roundRatio2(progress, progressTotal) } : {}
  };
}
...
function isUpgraderBoostStoredEnergySource(source) {
  return matchesStructureType9(source.structureType, "STRUCTURE_CONTAINER", "container") || matchesStructureType9(source.structureType, "STRUCTURE_STORAGE", "storage");
}
...
function isFillableEnergySink(structure) {
  return (matchesStructureType9(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType9(structure.structureType, "STRUCTURE_EXTENSION", "extension") || matchesStructureType9(structure.structureType, "STRUCTURE_TOWER", "tower")) && "store" in structure && getFreeStoredEnergyCapacity(structure) > 0;
}
...
function isRemoteHaulerStorageSink(structure) {
  return matchesStructureType9(structure.structureType, "STRUCTURE_STORAGE", "storage") && isRemoteHaulerStoredEnergySink(structure);
}
function isRemoteHaulerTerminalSink(structure) {
  return matchesStructureType9(structure.structureType, "STRUCTURE_TERMINAL", "terminal") && isRemoteHaulerStoredEnergySink(structure);
}
...
function isSpawnExtensionEnergyStructure(structure) {
  return (matchesStructureType9(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType9(structure.structureType, "STRUCTURE_EXTENSION", "extension")) && "store" in structure;
}
...
function isNearTermSpawningSpawn(structure) {
  var _a;
  if (!matchesStructureType9(structure.structureType, "STRUCTURE_SPAWN", "spawn")) {
    return false;
  }
...
function isTowerEnergySink(structure) {
  return matchesStructureType9(structure.structureType, "STRUCTURE_TOWER", "tower");
}
...
function findConstructionPriorityProtectedRampartAnchors(room) {
  ...
  for (const structure of findConstructionPriorityOwnedStructures(room)) {
    if (matchesStructureType9(structure.structureType, "STRUCTURE_SPAWN", "spawn") && structure.pos && isPositionInRoom(structure.pos, room.name)) {
...
function isSpawnConstructionSite(site) {
  return matchesStructureType9(site.structureType, "STRUCTURE_SPAWN", "spawn");
}
function isExtensionConstructionSite(site) {
  return matchesStructureType9(site.structureType, "STRUCTURE_EXTENSION", "extension");
}
function isContainerConstructionSite3(site) {
  return matchesStructureType9(site.structureType, "STRUCTURE_CONTAINER", "container");
}
function isRoadConstructionSite2(site) {
  return matchesStructureType9(site.structureType, "STRUCTURE_ROAD", "road");
}
function isRampartConstructionSite(site) {
  return matchesStructureType9(site.structureType, "STRUCTURE_RAMPART", "rampart");
}
...
function isStoredWorkerEnergySource(structure, room) {
  if (matchesStructureType9(structure.structureType, "STRUCTURE_LINK", "link")) {
    return isSourceLink(room, structure);
  }
  return matchesStructureType9(structure.structureType, "STRUCTURE_CONTAINER", "container") || matchesStructureType9(structure.structureType, "STRUCTURE_STORAGE", "storage") || matchesStructureType9(structure.structureType, "STRUCTURE_TERMINAL", "terminal");
}
...
function isFriendlyStoredEnergySource(structure, context) {
  ...
  return matchesStructureType9(structure.structureType, "STRUCTURE_CONTAINER", "container") && isRoomSafeForUnownedContainerWithdrawal(context);
}
...
function isStructureEnergySourceType(source, globalName, fallback) {
  ...
  return matchesStructureType9(typeof structureType === "string" ? structureType : void 0, globalName, fallback);
}
...
function findHostileStructures3(room) {
...
function isRoadRepairTarget(structure) {
  return matchesStructureType9(structure.structureType, "STRUCTURE_ROAD", "road");
}
...
function getWorkerRepairHitsCeiling(structure) {
  if (matchesStructureType9(structure.structureType, "STRUCTURE_RAMPART", "rampart") && isOwnedRampart(structure)) {
...
function hasVisiblePositionedContainer(room) {
  ...
  return room.find(FIND_STRUCTURES).some((structure) => {
    ...
    return position !== null && matchesStructureType9(structure.structureType, "STRUCTURE_CONTAINER", "container");
  });
}
...
function getHarvestSourceAccessCapacity(source) {
  ...
  const terrain = getRoomTerrain5(position.roomName);
  if (!terrain) {
    return 1;
  }
...
function isWalkableRampartStructure(structure) {
  return matchesStructureType9(structure.structureType, "STRUCTURE_RAMPART", "rampart") && (structure.my === true || structure.isPublic === true);
}
...
function observeCreepBehaviorTick(creep, tick = getGameTime9()) {
  var _a, _b;
...
function recordCreepBehaviorIdle(creep, tick = getGameTime9()) {
  var _a;
  const telemetry = ensureCreepBehaviorTelemetry(creep);
...
function recordCreepBehaviorMove(creep, tick = getGameTime9()) {
  var _a;
  const telemetry = ensureCreepBehaviorTelemetry(creep);
...
function recordCreepBehaviorWork(creep, tick = getGameTime9()) {
  var _a;
  const telemetry = ensureCreepBehaviorTelemetry(creep);
...
function getTaskTarget(task) {
  const game = globalThis.Game;
  const getObjectById4 = game == null ? void 0 : game.getObjectById;
  return typeof getObjectById4 === "function" ? getObjectById4(String(task.targetId)) : null;
}
...
function executeHarvestTask(creep, source) {
  ...
  if (!isInRangeToRoomObject(creep, source, 1)) {
    ...
    return { result: OK_CODE5, action: "move" };
  }
  if (isDepletedHarvestSource(source)) {
    return getUsedTransferEnergy(creep) > 0 ? transferDedicatedHarvestEnergy(creep, sourceContainer) : { result: OK_CODE5 };
  }
  ...
  return toTaskExecutionResult(result === ERR_NOT_ENOUGH_RESOURCES_CODE ? OK_CODE5 : result, "work");
}
function transferDedicatedHarvestEnergy(creep, sourceContainer) {
  if (typeof creep.transfer !== "function") {
    return { result: OK_CODE5 };
  }
...
function isRemoteBootstrapRecord(record, homeRoom) {
  return isRecord8(record) && record.colony === homeRoom && isNonEmptyString8(record.roomName) && record.roomName !== homeRoom && (record.status === "detected" || record.status === "spawnSitePending" || record.status === "spawnSiteBlocked" || record.status === "spawningWorkers" || record.status === "ready");
}
...
function hasHostileSuspendedTerritoryIntent(homeRoom, targetRoom) {
  ...
  return intents.some(
    (intent) => isRecord8(intent) && intent.colony === homeRoom && intent.targetRoom === targetRoom && intent.suspended !== void 0 && isRecord8(intent.suspended) && intent.suspended.reason === "hostile_presence"
  );
}
function normalizeRemoteHarvesterMemory(value) {
  ...
  return isNonEmptyString8(value.homeRoom) && isNonEmptyString8(value.targetRoom) && isNonEmptyString8(value.sourceId) && (value.containerId == null || isNonEmptyString8(value.containerId)) ? {
    homeRoom: value.homeRoom,
    targetRoom: value.targetRoom,
    sourceId: value.sourceId,
    ...isNonEmptyString8(value.containerId) ? { containerId: value.containerId } : {}
  } : null;
}
...
function getAssignedContainer(assignment) {
  if (isNonEmptyString8(assignment.containerId)) {
...
function findCriticalRoadMoveTargets(room) {
  return [
    ...findRoomObjects10(room, "FIND_STRUCTURES"),
    ...findRoomObjects10(room, "FIND_CONSTRUCTION_SITES")
  ].filter((target) => matchesStructureType10(target.structureType, "STRUCTURE_ROAD", "road"));
}
...
function getObjectById(id) {
  var _a;
  const getObjectById4 = (_a = globalThis.Game) == null ? void 0 : _a.getObjectById;
  return typeof getObjectById4 === "function" ? getObjectById4(String(id)) : null;
}
...
function hasRemoteContainerAssignment(assignment) {
  return isNonEmptyString9(assignment.containerId);
}
...
function normalizeRemoteHaulerMemory(value) {
  ...
  return isNonEmptyString9(value.homeRoom) && isNonEmptyString9(value.targetRoom) && isNonEmptyString9(value.sourceId) && isNonEmptyString9(value.containerId) ? {
    homeRoom: value.homeRoom,
    targetRoom: value.targetRoom,
    sourceId: value.sourceId,
...
function recordPlannedMultiRoomUpgraderSpawn(memory) {
  ...
  if (memory.role !== "worker" || (sustain == null ? void 0 : sustain.role) !== "upgrader" || !isNonEmptyString10(sustain.homeRoom) || !isNonEmptyString10(sustain.targetRoom)) {
    return;
  }
...
function getVisibleMultiRoomUpgradeCandidate(homeRoom, room, config, activeUpgraderCounts, order) {
  var _a;
  if (!isNonEmptyString10(room.name) || room.name === homeRoom || isKnownDeadZoneRoom(room.name)) {
    return null;
  }
  const controller = room.controller;
  if (!controller || !isNonEmptyString10(controller.id)) {
    return null;
  }
...
function isActiveSpawnConstructionSite(site) {
  return isRecord10(site) && matchesStructureType11(site.structureType, "STRUCTURE_SPAWN", "spawn") && hasIncompleteConstructionSiteProgress(site);
}
...
function countActiveMultiRoomUpgradersByHomeRoom(creeps) {
  ...
  for (const creep of Object.values(creeps)) {
    const sustain = (_a = creep.memory) == null ? void 0 : _a.controllerSustain;
    if ((sustain == null ? void 0 : sustain.role) !== "upgrader" || !isNonEmptyString10(sustain.homeRoom) || !isNonEmptyString10(sustain.targetRoom) || !isActiveMultiRoomUpgrader(creep)) {
      continue;
    }
...
function getTerritoryRouteDistanceCache2(gameTime) {
  ...
  if (!isRecord10(memory.territory.routeDistances)) {
    memory.territory.routeDistances = {};
  }
  if (!isRecord10(memory.territory.routeDistancesUpdatedAt)) {
...
// src/economy/storageBalancer.ts
...
function getRoomStoredEnergyState(room) {
  const stores = getRoomEnergyStores(room);
  const energy = stores.reduce((total, structure) => total + getStoredEnergy9(structure), 0);
...
function buildStorageBalanceState(gameTime) {
  const roomStates = getOwnedRooms().map(getRoomStoredEnergyState).filter((state) => state.capacity > 0);
  return {
    updatedAt: gameTime,
...
function compareImportRooms(left, right) {
  return right.importDemand - left.importDemand || left.ratio - right.ratio || left.roomName.localeCompare(right.roomName);
}
...
function getRoomEnergyStores(room) {
  const stores = [
    room.storage,
    room.terminal
  ];
...
function getStoredEnergy9(target) {
  ...
  const resource = getEnergyResource7();
...
function getEnergyCapacity2(target) {
  var _a, _b, _c;
  const store = target.store;
  const resource = getEnergyResource7();
  ...
  return typeof freeCapacity === "number" && Number.isFinite(freeCapacity) ? getStoredEnergy9(target) + Math.max(0, freeCapacity) : 0;
}
...
// src/economy/crossRoomHauler.ts
var CROSS_ROOM_HAULER_ROLE = "crossRoomHauler";
var CARRY_MOVE_PAIR_COST = 100;
var CARRY_CAPACITY_PER_PART = 50;
var MAX_CARRY_MOVE_PAIRS = 25;
var MIN_CARRY_MOVE_PAIRS = 1;
...
function planCrossRoomHauler() {
  ...
  const sourceRoom = getVisibleRoom4(transfer.sourceRoom);
  const targetRoom = getVisibleRoom4(transfer.targetRoom);
  ...
  return {
    spawn,
    body,
    name: `crossRoomHauler-${sourceRoom.name}-${targetRoom.name}-${getGameTime12()}`,
    memory: {
...
function runCrossRoomHauler(creep) {
...
function compareTransfers(left, right) {
  ...
  return right.amount - left.amount || leftRouteDistance - rightRouteDistance || left.sourceRoom.localeCompare(right.sourceRoom) || left.targetRoom.localeCompare(right.targetRoom);
}
...
function collectEnergy(creep, assignment) {
  ...
  if (((_a = creep.room) == null ? void 0 : _a.name) !== assignment.homeRoom) {
    delete creep.memory.task;
    moveTowardRoom3(creep, assignment, assignment.homeRoom);
    return;
  }
  ...
  if (result === getErrNotInRangeCode3()) {
    moveTo3(creep, source);
  }
}
function deliverEnergy2(creep, assignment) {
  var _a, _b;
  if (((_a = creep.room) == null ? void 0 : _a.name) !== assignment.targetRoom) {
    delete creep.memory.task;
    moveTowardRoom3(creep, assignment, assignment.targetRoom);
    return;
  }
...
function returnHome(creep, assignment) {
  var _a, _b;
  if (((_a = creep.room) == null ? void 0 : _a.name) !== assignment.homeRoom) {
    delete creep.memory.task;
    moveTowardRoom3(creep, assignment, assignment.homeRoom);
  ...
  const carriedEnergy = getCarriedEnergy4(creep);
  if (carriedEnergy > 0) {
    const homeTarget = selectHomeReturnTarget(creep.room);
    if (homeTarget) {
      const result = (_b = creep.transfer) == null ? void 0 : _b.call(creep, homeTarget, getEnergyResource8());
      if (result === getErrNotInRangeCode3()) {
...
function selectDeliveryTarget(room) {
  var _a;
  const targets = [
    ...findOwnedStructures4(room).filter(isSpawnOrExtensionWithDemand),
...
function selectHomeReturnTarget(room) {
  var _a;
  const targets = [room.storage, room.terminal].filter(
    (structure) => structure !== void 0 && getFreeEnergyCapacity5(structure) > 0
  );
  return (_a = targets.sort((left, right) => getObjectId5(left).localeCompare(getObjectId5(right)))[0]) != null ? _a : null;
}
function isSpawnOrExtensionWithDemand(structure) {
  return (matchesStructureType12(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType12(structure.structureType, "STRUCTURE_EXTENSION", "extension")) && getFreeEnergyCapacity5(structure) > 0;
}
function isContainerWithDemand(structure) {
  return matchesStructureType12(structure.structureType, "STRUCTURE_CONTAINER", "container") && getFreeEnergyCapacity5(structure) > 0;
}
...
function compareDeliveryTargets(left, right) {
  return getDeliveryPriority(right) - getDeliveryPriority(left) || getObjectId5(left).localeCompare(getObjectId5(right));
}
function getDeliveryPriority(target) {
  if (matchesStructureType12(target.structureType, "STRUCTURE_SPAWN", "spawn")) {
    return 3;
  }
  if (matchesStructureType12(target.structureType, "STRUCTURE_EXTENSION", "extension")) {
    return 2;
  }
  if (matchesStructureType12(target.structureType, "STRUCTURE_STORAGE", "storage")) {
    return 0;
  }
  if (matchesStructureType12(target.structureType, "STRUCTURE_TERMINAL", "terminal")) {
    return -1;
  }
  return 1;
}
function selectSourceEnergyStructure(room) {
  ...
  return (_a = [room.storage, room.terminal].filter(
    (structure) => structure !== void 0 && getStoredEnergy10(structure) > 0
  ).sort((left, right) => getStoredEnergy10(right) - getStoredEnergy10(left) || getObjectId5(left).localeCompare(getObjectId5(right)))[0]) != null ? _a : null;
}
function hasSourceSurplusEnergy(assignment) {
  const room = getVisibleRoom4(assignment.homeRoom);
  if (!room) {
    return false;
  }
  let source = getAssignedSource2(assignment);
  ...
  return getRoomStoredEnergyState(room).mode === "export" && source !== null && getStoredEnergy10(source) > 0;
}
function recoverSourceAssignment(creep, assignment) {
  var _a;
  const homeRoom = getVisibleRoom4(assignment.homeRoom);
...
function selectReplacementSource(assignment, room) {
...
function getAssignedSource2(assignment) {
  return assignment.sourceId ? getObjectById3(assignment.sourceId) : null;
}
function markSourceUnassigned(assignment) {
  assignment.sourceId = null;
  assignment.state = "unassigned";
}
function isSourceUnassigned(assignment) {
  return assignment.state === "unassigned" || !assignment.sourceId;
}
function selectSourceRoomSpawn(roomName) {
  var _a, _b;
  const spawns = (_a = globalThis.Game) == null ? void 0 : _a.spawns;
  if (!spawns) {
    return null;
  }
  return (_b = Object.values(spawns).filter((spawn) => {
    var _a2;
    return ((_a2 = spawn.room) == null ? void 0 : _a2.name) === roomName && !spawn.spawning;
  }).sort((left, right) => getSpawnEnergyScore(right) - getSpawnEnergyScore(left) || left.name.localeCompare(right.name))[0]) != null ? _b : null;
}
function getSpawnEnergyScore(spawn) {
  const room = spawn.room;
  return typeof (room == null ? void 0 : room.energyAvailable) === "number" && Number.isFinite(room.energyAvailable) ? room.energyAvailable : 0;
}
function findOwnedLogisticsRoute(fromRoom, targetRoom) {
  ...
  const route = gameMap.findRoute.call(gameMap, fromRoom, targetRoom, {
    routeCallback: (roomName) => isSafeLogisticsTransitRoom(roomName) ? 1 : Infinity
  });
...
function moveTowardRoom3(creep, assignment, destinationRoom) {
  var _a, _b;
  ...
  const visibleController = (_b = getVisibleRoom4(nextRoom)) == null ? void 0 : _b.controller;
  if (visibleController) {
  ...
  const RoomPositionCtor = globalThis.RoomPosition;
  if (typeof RoomPositionCtor === "function") {
    moveTo3(creep, new RoomPositionCtor(25, 25, nextRoom));
  }
}
...
function selectNextRouteRoom(currentRoom, assignment, destinationRoom, route) {
  var _a, _b, _c;
  if (!currentRoom) {
  ...
  if (destinationRoom === assignment.targetRoom) {
    if (currentRoom === assignment.homeRoom) {
      return (_a = route[0]) != null ? _a : assignment.targetRoom;
    }
    const routeIndex2 = route.indexOf(currentRoom);
    return routeIndex2 >= 0 ? (_b = route[routeIndex2 + 1]) != null ? _b : assignment.targetRoom : destinationRoom;
  }
  const routeIndex = route.indexOf(currentRoom);
  if (routeIndex > 0) {
    return (_c = route[routeIndex - 1]) != null ? _c : assignment.homeRoom;
  }
...
function hasHostilePresence(room) {
  const hostileCreepFind = getGlobalNumber6("FIND_HOSTILE_CREEPS");
  ...
  const hostileStructureFind = getGlobalNumber6("FIND_HOSTILE_STRUCTURES");
...
function findOwnedStructures4(room) {
  var _a, _b;
  const findMyStructures = getGlobalNumber6("FIND_MY_STRUCTURES");
...
function findRoomStructures2(room) {
  const findStructures = getGlobalNumber6("FIND_STRUCTURES");
...
function getMutableCrossRoomHaulerMemory(creep) {
  var _a;
  const normalized = normalizeCrossRoomHaulerMemory((_a = creep.memory) == null ? void 0 : _a.crossRoomHauler);
  if (!normalized) {
    return null;
  }
...
function getErrNotInRangeCode3() {
  var _a;
...
var DEFENDER_BODY_PATTERN_COST = 140;
var MAX_DEFENDER_BODY_PATTERN_COUNT = 5;
var HOSTILES_PER_DEFENDER = 3;
...
function getSpawnEnergyForecast(colony) {
  ...
  const incomingEnergy = transfers.filter((transfer) => transfer.targetRoom === colony.room.name).reduce((total, transfer) => total + normalizeNonNegativeInteger2(transfer.amount), 0);
  const outgoingEnergy = transfers.filter((transfer) => transfer.sourceRoom === colony.room.name).reduce((total, transfer) => total + normalizeNonNegativeInteger2(transfer.amount), 0);
  const energyAvailable = normalizeNonNegativeInteger2(colony.energyAvailable);
  return {
...
function shouldSuppressWorkerSpawnForCrossRoomImport(colony) {
  var _a, _b;
  const balance = getStorageBalanceMemory();
  if (!balance) {
    return false;
  }
  const roomBalance = (_a = balance == null ? void 0 : balance.rooms) == null ? void 0 : _a[colony.room.name];
  if ((roomBalance == null ? void 0 : roomBalance.mode) !== "import") {
    return false;
  }
...
function planLocalSurvivalSpawn(context) {
  if (context.workerCapacity >= context.workerTarget || !hasRecoveryWorkerSpawnEnergy(context.colony) || context.workerCapacity > 0 && shouldSuppressWorkerSpawnForCrossRoomImport(context.colony)) {
    return null;
  }
...
function buildDefenseBody(energyAvailable, hostileCount) {
  const desiredPatternCount = Math.max(1, Math.min(hostileCount, MAX_DEFENDER_BODY_PATTERN_COUNT));
  const affordablePatternCount = Math.floor(normalizeNonNegativeInteger2(energyAvailable) / DEFENDER_BODY_PATTERN_COST);
  const patternCount = Math.min(
    desiredPatternCount,
    affordablePatternCount,
    Math.floor(MAX_CREEP_PARTS4 / DEFENDER_BODY_PATTERN.length)
  );
  if (patternCount <= 0) {
    return [];
  }
...
function getDesiredDefenderCount(hostileCount) {
  return Math.max(1, Math.ceil(hostileCount / HOSTILES_PER_DEFENDER));
}
function getRoomHostileCreepCount(room) {
  const findHostiles = getGlobalNumber7("FIND_HOSTILE_CREEPS");
  if (findHostiles === void 0 || typeof room.find !== "function") {
    return 0;
  }
  const result = room.find(findHostiles);
  return Array.isArray(result) ? result.length : 0;
}
function countActiveRoomDefenders(roomName) {
  const game = globalThis.Game;
...
function hasActiveAttackPart2(creep) {
  ...
  const activeParts = (_a = creep.getActiveBodyparts) == null ? void 0 : _a.call(creep, attackPart);
  if (typeof activeParts === "number") {
    return activeParts > 0;
  }
  if (!Array.isArray(creep.body)) {
...
function selectPostClaimControllerSustainPlan(colony) {
  ...
  for (const record of records) {
    const targetRoom = getVisibleRoom5(record.roomName);
...
function isPostClaimControllerSustainRecord(record, colonyName) {
  return isRecord12(record) && record.colony === colonyName && record.roomName !== colonyName && isNonEmptyString12(record.roomName) && (record.status === "detected" || record.status === "spawnSitePending" || record.status === "spawnSiteBlocked" || record.status === "spawningWorkers" || record.status === "ready");
}
...
function buildRuntimeExpansionCandidates(colony) {
  ...
  for (const room of Object.values(rooms)) {
    if (!room || !isNonEmptyString13(room.name) || room.name === colonyName || ownedRoomNames.has(room.name)) {
      continue;
    }
    ...
    return [
      {
        ...
        ...room ? buildVisibleExpansionCandidateEvidence(room) : scoutIntel ? buildScoutedExpansionCandidateEvidence(scoutIntel) : buildUnseenExpansionCandidateEvidence(roomName)
      }
    ];
  });
}
...
function buildVisibleExpansionCandidateEvidence(room) {
  const controller = room.controller;
  const sources = findRoomObjects11(room, getFindConstant5("FIND_SOURCES"));
  const controllerSourceRange = calculateAverageControllerSourceRange2(controller, sources);
  const roomTerrain = getRoomTerrain6(room);
  ...
  const hostileCreepCount = findRoomObjects11(room, getFindConstant5("FIND_HOSTILE_CREEPS")).length;
  const hostileStructureCount = findRoomObjects11(
    room,
...
function buildScoutedExpansionCandidateEvidence(intel) {
  ...
  return {
    ...
    ...((_a = intel.controller) == null ? void 0 : _a.id) ? { controllerId: intel.controller.id } : {},
    sourceCount: intel.sourceCount,
    ...intel.sourceAccessPoints !== void 0 ? { sourceAccessPoints: intel.sourceAccessPoints } : {},
...
function scoreExpansionCandidate(input, candidate) {
  ...
  if (typeof candidate.controllerSourceRange === "number") {
    ...
    risks.push(
      visible || scouted ? "controller proximity evidence missing" : "controller proximity evidence missing until scout"
    );
    evidenceStatus = downgradeEvidenceStatus(evidenceStatus, "insufficient-evidence");
  }
  ...
  const hostileCreepCount = (_a = candidate.hostileCreepCount) != null ? _a : 0;
  const hostileStructureCount = (_b = candidate.hostileStructureCount) != null ? _b : 0;
  if (!visible && !scouted && (candidate.hostileCreepCount === void 0 || candidate.hostileStructureCount === void 0)) {
...
function getExpansionEvidenceAdjective(visible, scouted) {
...
function hasForeignControllerPresence(input, controller) {
  ...
  return isNonEmptyString13(controller.ownerUsername) && controller.ownerUsername !== input.colonyOwnerUsername || isNonEmptyString13(controller.reservationUsername) && controller.reservationUsername !== input.colonyOwnerUsername;
}
...
function persistExpansionCandidateScores(colony, report, gameTime) {
  ...
  const existingCandidates = Array.isArray(territoryMemory.expansionCandidates) ? territoryMemory.expansionCandidates.filter(
    (candidate) => isRecord13(candidate) && candidate.colony !== colony
  ) : [];
  const nextCandidates = report.candidates.slice(0, MAX_PERSISTED_EXPANSION_CANDIDATES).map((candidate, index) => toPersistedExpansionCandidateMemory(colony, candidate, gameTime, index + 1));
...
function upsertNextExpansionTarget(territoryMemory, target) {
  ...
  if (isRecord13(existingTarget) && existingTarget.createdBy === NEXT_EXPANSION_TARGET_CREATOR) {
...
function pruneNextExpansionTargets(colony, activeTarget, territoryMemory = getTerritoryMemoryRecord5()) {
  ...
  territoryMemory.targets = territoryMemory.targets.filter((target) => {
    ...
    if (isRecord13(target) && isNonEmptyString13(target.roomName) && target.action === "claim") {
...
function isNextExpansionTarget(target, colony) {
  return isRecord13(target) && target.colony === colony && target.action === "claim" && target.createdBy === NEXT_EXPANSION_TARGET_CREATOR;
}
...
function getTerritoryRouteDistanceCache3() {
  ...
  if (!isRecord13(territoryMemory.routeDistances)) {
    territoryMemory.routeDistances = {};
  }
  return territoryMemory.routeDistances;
}
...
function getFreshExpansionScoutIntel(colony, roomName, gameTime) {
  const intel = getTerritoryScoutIntel(colony, roomName);
  if (!intel || gameTime < intel.updatedAt) {
    return intel;
  }
...
function summarizeExpansionScoutController(controller) {
  return {
    ...controller.my === true ? { my: true } : {},
    ...controller.ownerUsername ? { ownerUsername: controller.ownerUsername } : {},
    ...controller.reservationUsername ? { reservationUsername: controller.reservationUsername } : {},
...
function calculateAverageControllerSourceRange2(controller, sources) {
  if (!(controller == null ? void 0 : controller.pos) || sources.length === 0) {
    return void 0;
  }
...
function calculateAverageSourceAccessPoints2(terrain, sources) {
  if (sources.length === 0) {
    return void 0;
  }
  if (!terrain || typeof terrain.get !== "function") {
...
function summarizeRoomTerrain(roomOrName) {
  return summarizeRoomTerrainFromTerrain2(getRoomTerrain6(roomOrName));
}
...
function refreshPostClaimBootstrap(colony, roleCounts, gameTime, telemetryEvents = []) {
  ...
  if (existingSpawnSite) {
    ...
    if (shouldReportExistingSite) {
      ...
      recordSpawnSitePlacedTelemetry(record, spawnSite, OK_CODE6, telemetryEvents, true);
    }
    return { active: true, spawnConstructionPending: true };
  }
  const sitePlan = planInitialSpawnConstructionSite(colony.room);
  const nextStatus = sitePlan.result === OK_CODE6 ? "spawnSitePending" : "spawnSiteBlocked";
  ...
  if (shouldReportSitePlan) {
    ...
    if (sitePlan.result === OK_CODE6 && sitePlan.position) {
...
function buildSpawnPlacementLookups(room, anchor, maximumScanRadius) {
  ...
  return {
    blockingPositions,
    mineralPositions,
    terrain: getRoomTerrain7(room.name)
  };
}
...
function findExistingSpawnConstructionSite(room) {
  var _a;
  const findConstant = getGlobalNumber8("FIND_MY_CONSTRUCTION_SITES");
  if (typeof room.find !== "function" || findConstant === null) {
    return null;
  }
  const sites = room.find(findConstant, {
    filter: (site) => matchesStructureType13(site.structureType, "STRUCTURE_SPAWN", "spawn")
  });
  return (_a = sites[0]) != null ? _a : null;
}
function findSources3(room) {
  const findConstant = getGlobalNumber8("FIND_SOURCES");
  if (typeof room.find !== "function" || findConstant === null) {
    return [];
  }
...
function getRoomObjectPosition4(object) {
  ...
  if (isRecord14(pos) && isFiniteNumber7(pos.x) && isFiniteNumber7(pos.y)) {
...
function isPostClaimBootstrapRecord(value, expectedRoomName) {
  return isRecord14(value) && value.roomName === expectedRoomName && isNonEmptyString14(value.colony) && isPostClaimBootstrapStatus(value.status) && isFiniteNumber7(value.claimedAt) && isFiniteNumber7(value.updatedAt);
}
...
function getTerrainWallMask6() {
  return typeof TERRAIN_MASK_WALL === "number" ? TERRAIN_MASK_WALL : DEFAULT_TERRAIN_WALL_MASK7;
}
...
function summarizeRoom(colony, colonyCreeps, persistOccupationRecommendations, eventMetrics, includeStructureSnapshot) {
  ...
  if (persistOccupationRecommendations) {
    persistOccupationRecommendationFollowUpIntent(territoryRecommendation, getGameTime15());
  }
  return {
    roomName: colony.room.name,
    energyAvailable: colony.energyAvailable,
    ...
    ...summarizeRuntimeBehavior(colonyWorkers, getGameTime15()),
    ...includeStructureSnapshot ? { structures: summarizeStructures(colony, colonyWorkers) } : {},
    ...summarizeWorkerEfficiency(colonyWorkers, getGameTime15()),
    ...summarizeRefillTelemetry(colonyWorkers, getGameTime15()),
    ...summarizeSpawnCriticalRefill(colonyWorkers, getGameTime15()),
...
function buildTerritoryIntentSummary(colonyName, roleCounts) {
  ...
  const suspendedTerritoryIntentCounts = getSuspendedTerritoryIntentCountsByRoom(colonyName, getGameTime15());
...
function isWorkerTaskBehaviorSample(value) {
  return isRecord15(value) && value.type === "workerTaskBehavior" && value.schemaVersion === 1 && typeof value.tick === "number" && Number.isFinite(value.tick) && typeof value.policyId === "string" && value.liveEffect === false && isRecord15(value.state) && isRecord15(value.action) && isWorkerTaskBehaviorActionType(value.action.type) && typeof value.action.targetId === "string";
}
...
function isWorkerTaskPolicyShadow(value) {
  return isRecord15(value) && value.type === "workerTaskPolicyShadow" && value.schemaVersion === 1 && typeof value.tick === "number" && Number.isFinite(value.tick) && typeof value.policyId === "string" && value.liveEffect === false && typeof value.matched === "boolean";
}
...
function countOwnedRamparts(structures) {
  return structures.filter((structure) => isRecord15(structure) && isObservedOwnedRampart(structure)).length;
}
...
function toRuntimeContainerSnapshot(structure) {
  const id = getObjectId6(structure);
  if (!id) {
    return null;
  }
  return {
...
function toRuntimeRepairTargetSnapshot(targetId, repairCount, structure) {
  const structureRecord = isRecord15(structure) ? structure : {};
...
function isStructureOfType(structure, globalName, fallback) {
  return isRecord15(structure) && matchesStructureType14(structure.structureType, globalName, fallback);
}
...
function isRefillDeliverySample(value) {
  return isRecord15(value) && typeof value.tick === "number" && Number.isFinite(value.tick) && typeof value.targetId === "string" && typeof value.deliveryTicks === "number" && Number.isFinite(value.deliveryTicks) && typeof value.activeTicks === "number" && Number.isFinite(value.activeTicks) && typeof value.idleOrOtherTaskTicks === "number" && Number.isFinite(value.idleOrOtherTaskTicks) && typeof value.energyDelivered === "number" && Number.isFinite(value.energyDelivered);
}
...
function isOwnedEnergyStoreStructure(structure) {
  ...
  return matchesStructureType14(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType14(structure.structureType, "STRUCTURE_EXTENSION", "extension") || matchesStructureType14(structure.structureType, "STRUCTURE_STORAGE", "storage") || matchesStructureType14(structure.structureType, "STRUCTURE_CONTAINER", "container") || matchesStructureType14(structure.structureType, "STRUCTURE_LINK", "link");
}
...
function getPendingBuildProgress(constructionSite) {
  if (!isRecord15(constructionSite)) {
    return 0;
  }
...
function getRepairBacklogHits(structure) {
  if (!isRecord15(structure) || !isObservableRepairBacklogStructure(structure)) {
    return 0;
  }
...
function isObservableRepairBacklogStructure(structure) {
  return matchesStructureType14(structure.structureType, "STRUCTURE_ROAD", "road") || matchesStructureType14(structure.structureType, "STRUCTURE_CONTAINER", "container") || isObservedOwnedRampart(structure);
}
function isObservedOwnedRampart(structure) {
  return matchesStructureType14(structure.structureType, "STRUCTURE_RAMPART", "rampart") && structure.my === true;
}
...
function summarizeRoomEventMetrics(room, refillTargetIds = getSpawnExtensionEnergyStructureIds(room)) {
  ...
  for (const entry of eventLog) {
    if (!isRecord15(entry) || typeof entry.event !== "number") {
      continue;
    }
    const data = isRecord15(entry.data) ? entry.data : {};
...
function isSpawnExtensionEnergyStructure2(structure) {
  return isRecord15(structure) && (matchesStructureType14(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType14(structure.structureType, "STRUCTURE_EXTENSION", "extension"));
}
...
function getRoomEventLog(room) {
  ...
  try {
    const eventLog = getEventLog.call(room, getEnergyResource9());
...
function getEnergyInStore(object) {
  if (!isRecord15(object) || !isRecord15(object.store)) {
    return 0;
  }
  const storedEnergy = object.store[getEnergyResource9()];
  if (typeof storedEnergy === "number") {
    return storedEnergy;
  }
  const getUsedCapacity = object.store.getUsedCapacity;
  if (typeof getUsedCapacity === "function") {
    const usedCapacity = getUsedCapacity.call(object.store, getEnergyResource9());
...
function getEnergyCapacityInStore(object) {
  ...
  if (typeof getCapacity === "function") {
    const capacity2 = getCapacity.call(object.store, getEnergyResource9());
  ...
  if (typeof getFreeCapacity === "function") {
    const freeCapacity = getFreeCapacity.call(object.store, getEnergyResource9());
...
function sumDroppedEnergy2(droppedResources) {
  const energyResource = getEnergyResource9();
  return droppedResources.reduce((total, droppedResource) => {
    if (!isRecord15(droppedResource) || droppedResource.resourceType !== energyResource) {
      return total;
    }
...
function isEnergyEventData(data) {
  return data.resourceType === void 0 || data.resourceType === getEnergyResource9();
}
...
function manageStorage(room) {
  ...
  const storageEnergy = getStoredEnergy11(storage);
  if (storageEnergy <= 0) {
    return { assignedTasks: 0, linkTransfers };
  }
...
function transferStorageLinkEnergy(room) {
  ...
  if (!controllerLink || !storageLink || getObjectId7(controllerLink) === getObjectId7(storageLink)) {
    return [];
  }
  if (getLinkCooldown2(storageLink) > 0) {
    return [];
  }
  const storedEnergy = getStoredEnergy11(storageLink);
  ...
  return [
    {
      amount,
      destinationId: getObjectId7(controllerLink),
      result: transferLinkEnergy2(storageLink, controllerLink, amount),
      sourceId: getObjectId7(storageLink)
    }
  ];
}
...
function findSpawnExtensionRefillTargets(room) {
  const structures = findOwnedStructures5(room).filter(
    (structure) => (matchesStructureType15(structure.structureType, "STRUCTURE_SPAWN", "spawn") || matchesStructureType15(structure.structureType, "STRUCTURE_EXTENSION", "extension")) && getFreeEnergyCapacity6(structure) > 0
  );
  ...
  return structures.map((target) => ({
    ...
    id: getObjectId7(target),
    priority: 3,
    target
  }));
}
function findTowerRefillTargets(room) {
  return findOwnedStructures5(room).filter(
    (structure) => matchesStructureType15(structure.structureType, "STRUCTURE_TOWER", "tower") && getStoredEnergy11(structure) < TOWER_REFILL_THRESHOLD && getFreeEnergyCapacity6(structure) > 0
  ).map((target) => ({
    freeCapacity: getFreeEnergyCapacity6(target),
    id: getObjectId7(target),
    priority: 2,
...
function findStorageManagementWorkers(room, storage, demandState) {
  const storageId = getObjectId7(storage);
    ...
    if ((task == null ? void 0 : task.type) === "transfer") {
      ...
      if (typeof remainingCapacity === "number") {
        demandState.remainingCapacityById.set(
          targetId,
          Math.max(0, remainingCapacity - getCarriedEnergy5(worker))
        );
      }
    }
...
function assignLoadedWorkers(workers, demandState) {
  let assignedTasks = 0;
  for (const worker of workers) {
    const carriedEnergy = getCarriedEnergy5(worker);
    if (carriedEnergy <= 0) {
      continue;
    }
...
function assignStorageWithdrawals(room, workers, storage, demandState, initialProjectedStorageEnergy) {
  ...
  for (const worker of workers) {
    if (!hasRemainingDemand(demandState) || getCarriedEnergy5(worker) > 0) {
      continue;
    }
...
function getRoomStorage2(room) {
  ...
  return (_a = findOwnedStructures5(room).filter(
    (structure) => matchesStructureType15(structure.structureType, "STRUCTURE_STORAGE", "storage")
  )[0]) != null ? _a : null;
}
function findOwnedStructures5(room) {
  if (typeof FIND_MY_STRUCTURES !== "number" || typeof room.find !== "function") {
    return [];
  }
  const result = room.find(FIND_MY_STRUCTURES);
...
function getStoredEnergy11(target) {
  var _a;
  const store = target == null ? void 0 : target.store;
  const storedEnergy = (_a = store == null ? void 0 : store.getUsedCapacity) == null ? void 0 : _a.call(store, getEnergyResource10());
  return typeof storedEnergy === "number" && Number.isFinite(storedEnergy) ? Math.max(0, storedEnergy) : 0;
}
...
function runRecommendedExpansionClaimExecutor(creep, telemetryEvents = []) {
  ...
  if (result === OK_CODE7 && isClaimVerified(assignment.targetRoom, controller)) {
    recordRecommendedClaimSuccess(creep, assignment, controller, telemetryEvents);
    completeClaimAssignment(creep);
    return true;
  }
  if (result === ERR_NOT_IN_RANGE_CODE7) {
    moveTowardController(creep, controller);
    return true;
  }
  if (hasClaimExecutionTimedOut(execution, gameTime)) {
...
function executeExpansionClaim(creep, controller, telemetryEvents = []) {
  var _a, _b, _c, _d, _e, _f, _g, _h;
  const result = typeof creep.claimController === "function" ? creep.claimController(controller) : OK_CODE7;
...
function evaluateAutonomousExpansionClaim(colony, report, gameTime, context, telemetryEvents) {
  ...
  const room = getVisibleRoom7(candidate.roomName);
  const controller = room == null ? void 0 : room.controller;
  if (room) {
    recordVisibleRoomScoutIntel(colonyName, room, gameTime, void 0, telemetryEvents);
  }
...
function buildAutonomousExpansionScoringInput(colony, report, context) {
  ...
  report.candidates.forEach((candidate, order) => {
    if (!isNonEmptyString15(candidate.roomName) || seenRooms.has(candidate.roomName)) {
      return;
    }
...
function summarizeExpansionController2(controller) {
  ...
  return {
    ...typeof controller.my === "boolean" ? { my: controller.my } : {},
    ...ownerUsername ? { ownerUsername } : {},
    ...isNonEmptyString15(reservationUsername) ? { reservationUsername } : {},
...
function getAdjacentRoomNames5(roomName, gameMap = ((_a) => (_a = globalThis.Game) == null ? void 0 : _a.map)()) {
  ...
  return EXIT_DIRECTION_ORDER4.flatMap((direction) => {
    const exitRoom = exits[direction];
    return isNonEmptyString15(exitRoom) ? [exitRoom] : [];
  });
}
function countActivePostClaimBootstraps2() {
  ...
  return Object.values(records).filter((record) => isRecord16(record) && record.status !== "ready").length;
}
...
function upsertTerritoryTarget2(territoryMemory, target) {
  ...
  const existingTarget = territoryMemory.targets.find(
    (rawTarget) => isSameTarget2(rawTarget, target) && isRecord16(rawTarget) && rawTarget.createdBy === target.createdBy
  );
  if (!existingTarget) {
    territoryMemory.targets.push(target);
    return;
  }
  if (isRecord16(existingTarget)) {
...
function pruneAutonomousExpansionClaimTargets(colony, territoryMemory = getTerritoryMemoryRecord6(), activeTarget, context) {
  ...
  territoryMemory.targets = territoryMemory.targets.filter((target) => {
    ...
    if (isRecord16(target) && isNonEmptyString15(target.roomName) && target.action === "claim") {
...
function pruneOccupationRecommendationTargets(territoryMemory, colony) {
  ...
  territoryMemory.targets = territoryMemory.targets.filter(
    (target) => !(isRecord16(target) && target.colony === colony && target.createdBy === "occupationRecommendation")
  );
}
...
function getVisibleHostileCreepCount2(targetRoom) {
  const room = getVisibleRoom7(targetRoom);
...
function selectClaimTargetController(creep, assignment) {
  ...
  return (_d = (_c = (_a = creep.room) == null ? void 0 : _a.controller) != null ? _c : (_b = getVisibleRoom7(assignment.targetRoom)) == null ? void 0 : _b.controller) != null ? _d : null;
}
...
function recordRecommendedClaimTerminalFailure(creep, assignment, result, reason, options) {
  ...
  if (options.suppressIntent) {
    suppressRecommendedClaimIntent(colony, assignment, getGameTime16(), options.controllerId, reason);
  }
  recordColonyExpansionClaimVerification({
    colony,
    targetRoom: assignment.targetRoom,
...
function recordRecommendedClaimRetry(creep, assignment, result, reason, options = {}) {
  ...
  updateRecommendedClaimIntentForRetry(colony, assignment, getGameTime16(), options);
}
...
function isMatchingRecommendedClaimTarget(target, colony, targetRoom) {
  return isRecord16(target) && target.colony === colony && target.roomName === targetRoom && target.action === "claim" && isTerritoryAutomationSource3(target.createdBy) && RECOMMENDED_EXPANSION_CLAIM_SOURCES.has(target.createdBy);
}
function recordColonyExpansionClaimVerification(input) {
  var _a;
  const colonyRoom = getVisibleRoom7(input.colony);
  const selection = (_a = colonyRoom == null ? void 0 : colonyRoom.memory) == null ? void 0 : _a.cachedExpansionSelection;
  if (!isRecord16(selection) || selection.targetRoom !== input.targetRoom) {
    return;
  }
...
function getVisibleClaimedRoom(targetRoom, controller) {
  ...
  const gameRoom = getVisibleRoom7(targetRoom);
...
function isAutonomousExpansionClaimTarget(target, colony) {
  return isRecord16(target) && target.colony === colony && target.action === "claim" && target.createdBy === AUTONOMOUS_EXPANSION_CLAIM_TARGET_CREATOR;
}
function isSameTarget2(left, right) {
  return isRecord16(left) && left.colony === right.colony && left.roomName === right.roomName && left.action === right.action;
}
...
function getControllerStatus2(room, scoutIntel) {
  ...
  if (room) {
    ...
    if (isNonEmptyString16((_b = controller.reservation) == null ? void 0 : _b.username)) {
      return "reserved";
    }
    return "neutral";
  }
...
function refreshAdjacentRoomReservationIntent(colony, gameTime = getGameTime17(), options = {}) {
...
function getReservationControllerState(colonyName, roomName, ownerUsername) {
  ...
  if (isNonEmptyString17(controller.reservationUsername)) {
    if (isNonEmptyString17(ownerUsername) && controller.reservationUsername === ownerUsername) {
      return {
        kind: "ownReserved",
...
function getVisibleControllerReservationState(controller, ownerUsername) {
  ...
  const reservation = controller.reservation;
  if (isNonEmptyString17(reservation == null ? void 0 : reservation.username)) {
    if (isNonEmptyString17(ownerUsername) && reservation.username === ownerUsername) {
      return {
        kind: "ownReserved",
        controllerId,
...
function isBlockingTerritoryTarget(target, colony) {
  return isRecord18(target) && target.colony === colony && target.enabled !== false && target.createdBy !== ADJACENT_ROOM_RESERVATION_TARGET_CREATOR && (target.action === "claim" || target.action === "reserve");
}
function isAdjacentRoomReservationTarget(target, colony) {
  return isRecord18(target) && target.colony === colony && target.action === "reserve" && target.createdBy === ADJACENT_ROOM_RESERVATION_TARGET_CREATOR;
}
...
function refreshColonyExpansionIntent(colony, assessment, gameTime = getGameTime18()) {
  const colonyName = colony.room.name;
...
function getColonyExpansionControllerState(colonyName, roomName, ownerUsername) {
  ...
  if (room) {
    ...
    if (controller2.my === true || isNonEmptyString18((_a = controller2.owner) == null ? void 0 : _a.username)) {
      return { kind: "owned", controllerId };
    }
    const reservationUsername = (_b = controller2.reservation) == null ? void 0 : _b.username;
    if (isNonEmptyString18(reservationUsername)) {
...
function hasBlockingClaimIntent(colony, targetRoom) {
  ...
  if (Array.isArray(territoryMemory.targets) && territoryMemory.targets.some(
    (target) => isRecord19(target) && target.colony === colony && target.action === "claim" && target.createdBy !== COLONY_EXPANSION_CLAIM_TARGET_CREATOR
  )) {
    return true;
  }
  return normalizeTerritoryIntents(territoryMemory.intents).some(
...
function pruneColonyExpansionClaimTargets(colony, territoryMemory, activeTarget) {
  ...
  if (Array.isArray(territoryMemory.targets)) {
    territoryMemory.targets = territoryMemory.targets.filter((target) => {
      if (!isRecord19(target) || target.colony !== colony || target.action !== "claim" || target.createdBy !== COLONY_EXPANSION_CLAIM_TARGET_CREATOR) {
        return true;
      }
      if (activeTarget && isSameTarget4(target, activeTarget)) {
        return true;
      }
      if (isNonEmptyString18(target.roomName)) {
...
function pruneAdjacentReservationForTarget(colony, targetRoom, territoryMemory) {
  if (Array.isArray(territoryMemory.targets)) {
    territoryMemory.targets = territoryMemory.targets.filter(
      (target) => !(isRecord19(target) && target.colony === colony && target.roomName === targetRoom && target.action === "reserve" && target.createdBy === "adjacentRoomReservation")
    );
  }
  const intents = normalizeTerritoryIntents(territoryMemory.intents).filter(
...
function runTerritoryControllerCreep(creep, telemetryEvents = []) {
  ...
  if (isTerritoryControlAction4(assignment.action) && typeof creep.attackController === "function" && canCreepPressureTerritoryController(creep, controller, creep.memory.colony)) {
    ...
    if (pressureResult === ERR_NOT_IN_RANGE_CODE8 && typeof creep.moveTo === "function") {
      creep.moveTo(controller);
      return;
    }
...
function tryFallbackClaimAssignmentToReserve(creep, assignment, controller) {
  ...
  if (reserveResult === ERR_NOT_IN_RANGE_CODE8 && typeof creep.moveTo === "function") {
    creep.moveTo(controller);
    return true;
  }
...
function selectTargetController(creep, assignment) {
  ...
  if (assignment.controllerId) {
    ...
    const getObjectById4 = game == null ? void 0 : game.getObjectById;
    if (typeof getObjectById4 === "function") {
      const controller = getObjectById4.call(game, assignment.controllerId);
      if (controller) {
        return controller;
      }
    }
...
function buildStrategyRecommendationRoomState(colony, creeps) {
  ...
  return {
    ...
    energyAvailable: colony.energyAvailable,
    energyCapacity: colony.energyCapacityAvailable,
    storedEnergy: getStoredEnergy12(room),
    sourceCount: sources.length,
    hostileCreepCount: hostileCreeps.length,
    hostileStructureCount: hostileStructures.length,
...
function getStoredEnergy12(room) {
...
function attemptCrossRoomHaulerSpawn(colonies, telemetryEvents, usedSpawnsByRoom, reservedSpawnEnergyByRoom) {
  ...
  const candidateSpawns = ((_b = sourceColony == null ? void 0 : sourceColony.spawns) != null ? _b : [spawnRequest.spawn]).filter((spawn) => !spawn.spawning && !usedSpawns.has(spawn));
...
function getAvailableSpawnEnergyAfterReservations(sourceColony, spawnRequest, reservedSpawnEnergyByRoom) {
  var _a, _b;
  const sourceRoomName = spawnRequest.spawn.room.name;
  const roomEnergy = (_a = sourceColony == null ? void 0 : sourceColony.energyAvailable) != null ? _a : spawnRequest.spawn.room.energyAvailable;
  return Math.max(0, roomEnergy - ((_b = reservedSpawnEnergyByRoom.get(sourceRoomName)) != null ? _b : 0));
}
...
function normalizeNextExpansionTargetSelection(rawSelection, colonyName) {
  ...
  if (rawSelection.status === "planned") {
    if (!isNonEmptyString19(rawSelection.targetRoom)) {
      return null;
    }
    return {
      status: "planned",
...
function recordUsedSpawn(usedSpawnsByRoom, roomName, spawn) {
  var _a;
  const usedSpawns = (_a = usedSpawnsByRoom.get(roomName)) != null ? _a : /* @__PURE__ */ new Set();
  usedSpawns.add(spawn);
  usedSpawnsByRoom.set(roomName, usedSpawns);
}
function recordReservedSpawnEnergy(reservedSpawnEnergyByRoom, roomName, bodyCost) {
  var _a;
  reservedSpawnEnergyByRoom.set(
    roomName,
    ((_a = reservedSpawnEnergyByRoom.get(roomName)) != null ? _a : 0) + bodyCost
  );
}
