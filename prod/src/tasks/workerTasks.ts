...
function selectLocalConstructionBacklogBeforeControllerSustainUpgradeTask(
  creep: Creep,
  controller: StructureController | undefined,
  constructionSites: ConstructionSite[],
  constructionReservationContext: ConstructionReservationContext
  ...
  if (
    ...
    !hasMinimumSpawnRecoveryEnergyForControllerSustainConstruction(creep.room) ||
    shouldUpgradeForRcl3DefenseUnlock(creep, controller)
  ) {
    return null;
  }
...
function hasMinimumSpawnRecoveryEnergyForControllerSustainConstruction(room: Room): boolean {
  const energyAvailable = getRoomEnergyAvailable(room);
...
function selectEmergencySpawnExtensionRefillTask(
  ...
  if (
    !spawnOrExtensionEnergySink ||
    !shouldKeepCurrentWorkerForEmergencySpawnExtensionRefill(creep, spawnOrExtensionEnergySink)
  ) {
    return null;
  }
...
function shouldKeepCurrentWorkerForEmergencySpawnExtensionRefill(
  creep: Creep,
  spawnOrExtensionEnergySink: StructureSpawn | StructureExtension | null
): boolean {
  if (!hasEmergencySpawnExtensionRefillDemand(creep)) {
...
function isMinimumWorkerSpawnEnergyCoveredByOtherRefillWorkers(creep: Creep): boolean {
...
function shouldKeepSpawnExtensionRefillBeforeBootstrapExtension(
  ...
  return (
    ...
    (shouldKeepCurrentWorkerForEmergencySpawnExtensionRefill(creep, spawnOrExtensionEnergySink) ||
      isCriticalSpawnEnergySink(spawnOrExtensionEnergySink))
  );
}
...
function shouldYieldSpawnReservationToConstructionBacklog(
  ...
  if (
    hasEmergencySpawnExtensionRefillDemand(creep) &&
    shouldKeepCurrentWorkerForEmergencySpawnExtensionRefill(creep, selectSpawnOrExtensionEnergySink(creep))
  ) {
    return false;
  }
...
function selectLowLoadConstructionCoverageTask(
  creep: Creep,
  constructionSites: ConstructionSite[],
  constructionReservationContext: ConstructionReservationContext,
  getShouldYieldSpawnReservationToConstructionBacklog: () => boolean
...
function selectEmergencyOwnedRampartRepairTarget(creep: Creep): StructureRampart | null {
  ...
  return (
    repairTargets.find((structure) => hasEmergencyRampartRepairAssignmentCapacity(creep, structure)) ??
    repairTargets[0]
  );
}
...
function hasEmergencyRampartRepairAssignmentCapacity(creep: Creep, structure: StructureRampart): boolean {
  return hasLoadedRampartRepairAssignmentCapacity(creep, structure);
}

function hasLoadedRampartRepairAssignmentCapacity(creep: Creep, structure: StructureRampart): boolean {
  return (
    isWorkerAssignedToRepairTarget(creep, structure) ||
...
function selectCoveredSurplusControllerProgressTask(
  creep: Creep,
  controller: StructureController,
  constructionSites: ConstructionSite[]
): Extract<CreepTaskMemory, { type: 'upgrade' }> | null {
  if (
    !canLevelUpController(controller) ||
    !shouldUseSurplusForControllerProgress(creep, controller) ||
    !hasReserveSafeControllerProgressEnergy(creep) ||
    !hasCoveredNonControllerProductiveDemandForSurplusProgress(creep, constructionSites)
  ) {
    return null;
  }
...
function hasReserveSafeControllerProgressEnergy(creep: Creep): boolean {
  ...
  const availableEnergy = getStorageEnergyAvailableForWithdrawal(creep.room, storage, getStoredEnergy(storage));
  return availableEnergy >= Math.max(1, getEnergyCapacity(creep));
}

function hasCoveredNonControllerProductiveDemandForSurplusProgress(
  creep: Creep,
  constructionSites: ConstructionSite[]
): boolean {
  if (
    hasOwnedConstructionDemand(constructionSites) &&
    !hasOtherSameRoomLoadedBuildWorker(creep)
  ) {
    return false;
  }
...
function hasOwnedConstructionDemand(constructionSites: ConstructionSite[]): boolean {
