...
export function isTerritoryScoutAssignmentAvailableForCreep(
  ...
  if (targetAssignmentCount > 0) {
    return (
      targetTimedOut &&
      targetAssignmentCount < 2 &&
      assignments.length < TERRITORY_SCOUT_MAX_ACTIVE_ASSIGNMENTS
    );
  }
...
function getActiveScoutAssignmentCapacityUsage(assignments: ActiveScoutAssignment[]): number {
  const activeTargetRooms = new Set<string>();
