...
describe('historical replay validator', () => {
  ...
  it('loads historical replay skeleton data from Memory by room', () => {
    ...
    expect(loadHistoricalReplays('E17S59')).toEqual([replay('stored-1', 120, [12])]);
...
function replay(replayId: string, finalScore: number, strategyScores: number[], room = 'E17S59'): HistoricalReplay {
  return {
    replayId,
    room,
    startTick: 1,
