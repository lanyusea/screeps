...
describe('runEconomy', () => {
  ...
  it('refreshes next expansion scoring before the interval when owned room count reaches the RCL cap', () => {
    ...
    expect(getRoomTerrain).toHaveBeenCalledTimes(10);
    expect(room.memory.lastExpansionScoreTime).toBe(502);
    expect(room.memory.cachedExpansionSelection).toMatchObject({
      status: 'skipped',
      colony: 'W1N1',
  ...
  it('spawns a cross-room worker for spawnless E17S58 when claimed sources and containers are visible', () => {
    ...
    const claimedRoom = makeClaimedSpawnlessEconomyRoom({ roomName: 'E17S58', sourceCount: 2 });
    const spawn = {
      name: 'Spawn1',
      room: homeRoom,
      spawning: null,
    ...
    (globalThis as unknown as { Game: Partial<Game> }).Game = {
      time: 818,
      rooms: { E17S59: homeRoom, E17S58: claimedRoom },
      spawns: { Spawn1: spawn },
      creeps: {
        Worker1: makeEconomyWorker(homeRoom),
        Worker2: makeEconomyWorker(homeRoom),
    ...
    expect(spawn.spawnCreep).toHaveBeenCalledWith(['work', 'carry', 'move'], 'worker-E17S58-818', {
      memory: {
        role: 'worker',
        colony: 'E17S58',
        spawnSupport: { originRoom: 'E17S59', targetRoom: 'E17S58' }
      }
    });
  });

  it('keeps E17S59 defender spawning ahead of E17S58 economic recovery', () => {
    installMultiRoomSpawnQueueGlobals();
    ...
    expect(spawn.spawnCreep).toHaveBeenCalledTimes(1);
    expect(spawn.spawnCreep).toHaveBeenCalledWith(['tough', 'attack', 'move'], 'defender-E17S59-819', {
      memory: {
        role: 'defender',
        colony: 'E17S59',
        defense: { homeRoom: 'E17S59' }
      }
    });
  });
  ...
  it('uses the nearest viable spawn room for a spawnless colony worker deficit', () => {
    installMultiRoomSpawnQueueGlobals();
    (globalThis as unknown as { Memory: Partial<Memory> }).Memory = {};
    const targetRoom = makeClaimedSpawnlessEconomyRoom({ roomName: 'E17S58', sourceCount: 1 });
    ...
    (globalThis as unknown as { Game: Partial<Game> }).Game = {
      time: 820,
      rooms: { E17S58: targetRoom, W9N9: farRoom, E17S59: nearRoom },
    ...
    expect(nearSpawn.spawnCreep).toHaveBeenCalledWith(['work', 'carry', 'move'], 'worker-E17S58-820', {
      memory: {
        role: 'worker',
        colony: 'E17S58',
        spawnSupport: { originRoom: 'E17S59', targetRoom: 'E17S58' }
  ...
  it('registers all visible E17S58 sources for post-claim remote mining from E17S59', () => {
    (globalThis as unknown as {
      FIND_SOURCES: number;
      FIND_STRUCTURES: number;
      FIND_CONSTRUCTION_SITES: number;
    ...
    const homeRoom = makeTerritoryReadyEconomyRoom();
    (homeRoom as Room & { name: string }).name = 'E17S59';
    const constructionSites: ConstructionSite[] = [];
    const claimedRoom = {
      name: 'E17S58',
      energyAvailable: 0,
      ...
      controller: {
        ...
        my: true,
        level: 1,
        pos: { x: 25, y: 25, roomName: 'E17S58' } as RoomPosition
      } as StructureController,
      find: jest.fn((type: number) => {
        if (type === FIND_SOURCES) {
          return [
            { id: 'e17s58-source-a', pos: { x: 10, y: 10, roomName: 'E17S58' } as RoomPosition } as Source,
            { id: 'e17s58-source-b', pos: { x: 35, y: 35, roomName: 'E17S58' } as RoomPosition } as Source
          ];
        }
      ...
      createConstructionSite: jest.fn((x: number, y: number, structureType: StructureConstant) => {
        constructionSites.push({
          id: `site-${x}-${y}`,
          structureType,
          pos: { x, y, roomName: 'E17S58' } as RoomPosition
        } as ConstructionSite);
        return OK_CODE;
      })
    } as unknown as Room & { createConstructionSite: jest.Mock };
    ...
    expect(Object.keys(Memory.territory?.remoteMining?.['E17S59:E17S58']?.sources ?? {})).toEqual([
      'e17s58-source-a',
      'e17s58-source-b'
    ]);
    expect(Memory.territory?.remoteMining?.['E17S59:E17S58']).toMatchObject({
      colony: 'E17S59',
      roomName: 'E17S58',
      status: 'containerPending',
      sources: {
