...
const TEST_FULL_SPAWN_ENERGY = 300;
...
function makeFullSpawnEnergyStructure(
  id: string,
  roomName = 'W1N1',
  extra: Record<string, unknown> = {}
): StructureSpawn {
  return makeEnergySinkWithEnergy(id, 'spawn' as StructureConstant, TEST_FULL_SPAWN_ENERGY, 0, {
    my: true,
...
function installInterRoomEnergyGame(
  sourceRoom: Room,
  targetRoom: Room,
  ...
  (globalThis as unknown as { Memory: Partial<Memory> }).Memory = {
    economy: {
      storageBalance: {
        updatedAt: 500,
        rooms: {},
        transfers: [{ sourceRoom: 'W1N1', targetRoom: 'W2N1', amount: 100, updatedAt: 500 }]
      }
    }
  };
}
...
describe('selectWorkerTask', () => {
  ...
  it('keeps normal productive spending before surplus storage when spawn extensions and containers are full', () => {
    const storage = makeEnergySinkWithEnergy(
      'storage1',
      STRUCTURE_STORAGE,
      0,
      1_000,
      { hits: 1_000, hitsMax: 1_000 }
  ...
  it('withdraws from source storage for a planned inter-room energy haul', () => {
  ...
  it('delivers carried energy to target room storage when local productive work is exhausted', () => {
    const { sourceRoom, targetRoom } = makeInterRoomEnergyRooms();
    installInterRoomEnergyGame(sourceRoom, targetRoom);
    const creep = {
      memory: { role: 'worker', colony: 'W1N1' },
  ...
  it('keeps local repair before inter-room energy hauling', () => {
    ...
    expect(selectWorkerTask(creep)).toEqual({ type: 'repair', targetId: 'road1' });
  });
  ...
  it('repairs near-floor owned ramparts before normal construction', () => {
    ...
    expect(selectWorkerTask(creep)).toEqual({ type: 'repair', targetId: 'rampart-near-floor' });
  });

  it('spreads emergency owned rampart repair to uncovered low-hit targets', () => {
    const controller = {
      id: 'controller1',
      my: true,
      level: 6,
    ...
    const lowestRampart = makeStructure(
      'rampart-lowest',
      'rampart' as StructureConstant,
      3_601,
      30_000_000,
    ...
    const nextRampart = makeStructure(
      'rampart-next',
      'rampart' as StructureConstant,
      3_701,
      30_000_000,
    ...
    const thirdRampart = makeStructure(
      'rampart-third',
      'rampart' as StructureConstant,
      4_101,
      30_000_000,
    ...
    const room = makeWorkerTaskRoom({
      ...
      energyAvailable: 550,
      energyCapacityAvailable: 2_300,
      structures: [lowestRampart, nextRampart, thirdRampart]
    });
    const coveringRepairer = {
      name: 'CoveringRepairer',
      memory: {
        role: 'worker',
        colony: 'E29N55',
        task: { type: 'repair', targetId: 'rampart-lowest' as Id<Structure> }
      },
      store: { getUsedCapacity: jest.fn().mockReturnValue(100) },
      room
    } as unknown as Creep;
    ...
    setGameCreeps({ CoveringRepairer: coveringRepairer, SecondRepairer: creep });

    expect(selectWorkerTask(creep)).toEqual({ type: 'repair', targetId: 'rampart-next' });
  });

  it('keeps emergency owned rampart repair ahead of non-critical tower refill', () => {
    const controller = {
      id: 'controller1',
      my: true,
      level: 6,
  ...
  it('builds capacity extension construction once source logistics starvation has cleared', () => {
    const source = makeSource('source1', 20, 10);
    const sourceContainerSite = {
      id: 'source-container-site1',
      structureType: 'container',
  ...
  it('assigns a minimum harvester before capacity construction when spawn energy is below capacity', () => {
    ...
    setGameCreeps({ OtherBuilder: otherBuilder, RecoveryHarvester: creep });

    expect(selectWorkerTask(creep)).toEqual({ type: 'harvest', targetId: 'source1' });
  });

  it('keeps capacity construction available once another worker is harvesting', () => {
    const source = makeSource('source1', 20, 10);
    const site = { id: 'extension-site1', structureType: 'extension' } as ConstructionSite;
    const controller = {
      id: 'controller1',
    ...
    const assignedHarvester = {
      name: 'AssignedHarvester',
      memory: { role: 'worker', task: { type: 'harvest', targetId: 'source1' as Id<Source> } },
      store: {
        getUsedCapacity: jest.fn().mockReturnValue(0),
        getFreeCapacity: jest.fn().mockReturnValue(50)
    ...
    setGameCreeps({ AssignedHarvester: assignedHarvester, Builder: creep });

    expect(selectWorkerTask(creep)).toEqual({ type: 'build', targetId: 'extension-site1' });
  });

  it('assigns a minimum harvester when build-assigned workers have no carried energy', () => {
    const source = makeSource('source1', 20, 10);
    const site = { id: 'extension-site1', structureType: 'extension' } as ConstructionSite;
    const controller = {
      id: 'controller1',
    ...
    setGameCreeps({ EmptyBuilder: emptyBuilder, RecoveryHarvester: creep });
  ...
  it('keeps E29N57 source-container construction productive during low-bucket shedding when stored energy protects recovery', () => {
    ...
    const fullSpawn = makeEnergySink('spawn1', 'spawn' as StructureConstant, 0);
    const storedContainer = makeStoredEnergyContainerWithCapacity('stored-container1', 2_217, 2_500);
    const controller = {
      id: 'controller1',
      my: true,
      level: 5,
  ...
  it('uses a loaded E29N56 local sustain upgrader on visible construction when spawn recovery energy is preserved', () => {
    const site = {
      id: 'wall-site1',
      my: true,
      structureType: 'constructedWall',
    ...
    const room = makeWorkerTaskRoom({
      ...
      constructionSites: [site],
      controller,
      energyAvailable: TEST_FULL_SPAWN_ENERGY,
      energyCapacityAvailable: TEST_FULL_SPAWN_ENERGY,
      myStructures: [spawn as AnyOwnedStructure],
      structures: [spawn as AnyStructure]
    });
    const sustainUpgrader = {
  ...
  it('keeps critical CPU repair before E29N56 local sustain construction', () => {
    ...
    const room = makeWorkerTaskRoom({
      ...
      energyCapacityAvailable: TEST_FULL_SPAWN_ENERGY,
      myStructures: [spawn as AnyOwnedStructure],
      structures: [spawn as AnyStructure, criticalContainer]
    });
    const sustainUpgrader = {
      name: 'worker-E29N56-2171723',
      memory: {
    ...
    expect(selectWorkerTask(sustainUpgrader)).toEqual({ type: 'repair', targetId: 'container-critical' });
  });
  ...
  it('acquires energy for defense-floor rampart repair while construction sites remain', () => {
    const controller = {
      id: 'controller1',
      my: true,
      level: 6,
    ...
    expect(selectActiveRampartRepairEnergyAcquisitionTask(creep)).toEqual({
      type: 'harvest',
      targetId: 'source1'
    });
  });
