...
def apply_runtime_parameter_injection_to_code(code_text: str, injection: JsonObject) -> str:
    ...
    dedupe_global = f"{RUNTIME_PARAMETER_CONSUMPTION_GLOBAL}_LAST_SERIALIZED"
    prelude = (
        ...
        f"  var dedupeGlobal = {json.dumps(dedupe_global)};\n"
        ...
        "      var dedupeRoot = roots[0];\n"
        "      var lastMaterializedConsumption = dedupeRoot ? dedupeRoot[dedupeGlobal] : null;\n"
        "      if (serialized !== lastMaterializedConsumption) {\n"
        "        if (dedupeRoot) dedupeRoot[dedupeGlobal] = serialized;\n"
    ...
    epilogue = (
        ...
        "    } catch (err) {}\n"
        "  }\n"
        "  function materializeKnownRuntimeConsumptionEvidence() {\n"
        "    materializeRuntimeConsumptionEvidence(lexicalConsumptionEvidence());\n"
        "    for (var index = 0; index < roots.length; index += 1) {\n"
        "      try { materializeRuntimeConsumptionEvidence(roots[index][consumptionGlobal]); } catch (err) {}\n"
        "    }\n"
        "    try {\n"
        "      var memory = runtimeMemoryRoot();\n"
        "      if (memory) materializeRuntimeConsumptionEvidence(memory.rlRuntimePolicyParameters);\n"
        "    } catch (err) {}\n"
        "  }\n"
        "  function markWrapped(target) {\n"
        "    try {\n"
        "      Object.defineProperty(target, '__screepsRlRuntimePolicyConsumptionWrapped', {\n"
        "        value: true,\n"
        "        configurable: true\n"
        "      });\n"
        "    } catch (err) { target.__screepsRlRuntimePolicyConsumptionWrapped = true; }\n"
        "  }\n"
        "  function wrapFunction(originalLoop) {\n"
        "    if (originalLoop.__screepsRlRuntimePolicyConsumptionWrapped) return originalLoop;\n"
        "    var wrappedLoop = function() {\n"
        "      try {\n"
        "        return originalLoop.apply(this, arguments);\n"
        "      } finally {\n"
        "        materializeKnownRuntimeConsumptionEvidence();\n"
        "      }\n"
        "    };\n"
        "    try {\n"
        "      for (var key in originalLoop) {\n"
        "        if (Object.prototype.hasOwnProperty.call(originalLoop, key)) wrappedLoop[key] = originalLoop[key];\n"
        "      }\n"
        "    } catch (err) {}\n"
        "    markWrapped(wrappedLoop);\n"
        "    return wrappedLoop;\n"
        "  }\n"
        "  function wrapLoop(container) {\n"
        "    if (!container || container.__screepsRlRuntimePolicyConsumptionWrapped) return container;\n"
        "    if (typeof container === 'function') return wrapFunction(container);\n"
        "    if (typeof container.loop === 'function') {\n"
        "      container.loop = wrapFunction(container.loop);\n"
        ...
        "      module.exports = wrapLoop(module.exports);\n"
        "      if (module.exports.default) module.exports.default = wrapLoop(module.exports.default);\n"
        "    }\n"
        "  } catch (err) {}\n"
        "})();\n"
    )
    insert_at = runtime_parameter_injection_insert_index(code_text)
    return code_text[:insert_at] + prelude + code_text[insert_at:] + epilogue
