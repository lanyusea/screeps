...
function readRoadmapData() {
  ...
  for (const key of ['kpiCards', 'roadmapCards', 'domainKanban', 'processCards']) {
...
function roadmapCard(card) {
  const hasProgress = card.progress !== null && card.progress !== undefined && card.progress !== '';
  const progress = hasProgress ? displayValue(card.progress) : 'n/a';
  const fill = hasProgress ? clampPercent(card.progress) : 0;
  const suffix = hasProgress ? '%' : '';
  const body = `<h3>${esc(card.title)}</h3><div class="row"><b>Goal</b><span>${esc(card.goal)}</span></div><div class="row"><b>Next</b><span>${esc(card.next)}</span></div><div class="progress"><strong>${esc(progress)}${suffix}</strong><i><em style="width:${fill}%"></em></i></div><div class="done">${esc(card.status)}</div>`;
  if (card.url) {
...
<section class="section"><div class="section-title"><h2>02 Project Domains</h2></div><div class="road-grid">${report.roadmapCards.map(roadmapCard).join('')}</div></section>
${kanban('03 Project Domain Board', report.domainKanban)}
<section class="section"><div class="section-title"><h2>04 Delivery Metrics</h2></div><div class="process-grid">${report.processCards.map(metric).join('')}</div></section>
